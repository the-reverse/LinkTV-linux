...
/* From Mars on, revision number would be A0, B0, C0, ... */
				...
				else if(chip_info == 0xC0)
					platform_info.cpu_id = realtek_marsC_cpu;
				else if(chip_info == 0xD0)
					platform_info.cpu_id = realtek_marsD_cpu;
				else {
					if(!state)
						state = 2;
					platform_info.cpu_id = realtek_mars_cpu;
...
/* From Mars on, revision number would be A0, B0, C0, ... */
				...
				if(chip_info == 0xA0)
					...
					platform_info.cpu_id = realtek_jupiterB_cpu;
				else if(chip_info == 0xC0)
					platform_info.cpu_id = realtek_jupiterC_cpu;
				else if(chip_info == 0x04E0)	// A strange value.
					platform_info.cpu_id = realtek_jupiterD_cpu;
				else {
					if(!state)
						state = 2;
					platform_info.cpu_id = realtek_jupiter_cpu;
...
//                prom_printf("\n\tchip_bound_opt=%x\n",chip_bound_opt);
	...
	parse_variable("usb_internal_port", platform_info.usb_internal_port, 8, "");
