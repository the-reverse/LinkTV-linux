...
{
/* For external timer interrupt, we clear MIS_ISR in realtek_timer_ack. For internal timer interrupt, MIPS has built-in function, realtek_timer_ack, to handle Count and Compare registers. */
...
int venus_rtc_alarm_set_time(unsigned long second)
{
	...
	if(is_jupiter_cpu()) {
		outl(0x1, OFFSET_JUPITER_ISO_RTC);
		outl(0x2000, OFFSET_JUPITER_ISO_ISR);
	} else {
