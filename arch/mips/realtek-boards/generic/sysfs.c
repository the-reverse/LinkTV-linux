...
#ifdef CONFIG_REALTEK_LOGO_PARAM
extern logo_info_struct logo_info;
#endif
#if CONFIG_REALTEK_RESERVE_DVR && !CONFIG_REALTEK_AUTO_RECLAIM
...
#if CONFIG_REALTEK_RESERVE_DVR && !CONFIG_REALTEK_AUTO_RECLAIM
static ssize_t reclaim_dvr_show(struct subsystem *subsys, char *page)
{
	return sprintf(page, "DVR is reserved: %d\n", reserveDVR);
}

static ssize_t reclaim_dvr_store(struct subsystem *subsys, const char *page, size_t count)
{
	char *p, buffer[20] = {0};
	int len, cmd;
	int aflag = 0, vflag = 0, vStat = 0;
	volatile char *ptr = (char *)ntohl(*(unsigned long *)SYNC_STRUCT_ADDR);
	...
	if ((cmd == 1) && (reserveDVR == 1)) {
		if (ptr) {
			printk("=====check animation logo...\n");
			while (1) {
				if (vStat != 2) {
					// check video flag...
					if (2 == ptr[20]) { //video playback is halt or finished.
						if (vStat == 0) { //video is halt at last frame.
							ptr[20] = 1; //notify video f/w to stop playback.
							if (2 != ptr[21]) //if we have video only. audio f/w sets it to 2 directly.
								ptr[21] = 1; //notify audio f/w to stop playback concurrently.
							vStat = 1;
						}
						else if (vStat == 1) { //finished.
							vflag = 1;
							vStat = 2;
				...
				if (vflag && !aflag) { //after video is ready, check audio status
					// check audio flag...
					if (2 == ptr[21]) {
						aflag = 1;
...
static ssize_t usb_internal_port_show(struct subsystem *subsys, char *page)
{
	return sprintf(page, "%s\n", platform_info.usb_internal_port);
}
REALTEK_BOARDS_ATTR_RO(usb_internal_port);
...
#if CONFIG_REALTEK_CPU_VENUS || CONFIG_REALTEK_CPU_NEPTUNE
			setup_boot_image();
#else
			;
#endif
