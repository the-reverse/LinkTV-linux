...
#ifdef DEV_DEBUG
	printk(KERN_WARNING "Venus IR: reg_ir_sr = [%08X]\n", srValue);
#endif
...
#ifdef DEV_DEBUG
		printk(KERN_WARNING "Venus IR: reg_ir_rp = [%08X]\n", *regValue);
#endif
...
#endif
		iowrite32(0x00000020, reg_isr); /* clear interrupt flag */
...
static int Venus_IR_Init(int mode) {
	...
	switch(mode) {
		...
		case SHARP:
			iowrite32(0x80000000, reg_ir_cr);
			iowrite32(0x000c1b54, reg_ir_psr);
			iowrite32(0x00010000, reg_ir_per);
			iowrite32(0x21b, reg_ir_sf);
			iowrite32(0xc8, reg_ir_dpir);
		...
		case RC6:
			...
			iowrite32(0x21b, reg_ir_sf);
			iowrite32(0x3ff, reg_ir_dpir);
			iowrite32(0x515, reg_ir_cr); // valid bit length is 21
			if(is_jupiter_cpu())
		...
		case RAW_NEC:
			...
			iowrite32(0x0a8b, reg_ir_raw_deb);	// 2x over sampling for debounce
...
int venus_ir_init_module(void) {
	...
	{
		reg_isr                = ISO_ISR;
		reg_ir_psr             = ISO_IR_PSR;
		reg_ir_per             = ISO_IR_PER;
		reg_ir_sf              = ISO_IR_SF;
		reg_ir_dpir            = ISO_IR_DPIR;
		reg_ir_cr              = ISO_IR_CR;
		reg_ir_rp              = ISO_IR_RP;
		reg_ir_sr              = ISO_IR_SR;
		reg_ir_raw_ctrl        = ISO_IR_RAW_CTRL;
		reg_ir_raw_ff          = ISO_IR_RAW_FF;
		reg_ir_raw_sample_time = ISO_IR_RAW_SAMPLE_TIME;
		reg_ir_raw_wl          = ISO_IR_RAW_WL;
		reg_ir_raw_deb         = ISO_IR_RAW_DEB;
		reg_ir_ctrl_rc6        = ISO_IR_CTRL_RC6;
	}
	else
	{
		reg_isr                = MIS_ISR;
		reg_ir_psr             = MIS_IR_PSR;
		reg_ir_per             = MIS_IR_PER;
		reg_ir_sf              = MIS_IR_SF;
		reg_ir_dpir            = MIS_IR_DPIR;
		reg_ir_cr              = MIS_IR_CR;
		reg_ir_rp              = MIS_IR_RP;
		reg_ir_sr              = MIS_IR_SR;
		reg_ir_raw_ctrl        = MIS_IR_RAW_CTRL;
		reg_ir_raw_ff          = MIS_IR_RAW_FF;
		reg_ir_raw_sample_time = MIS_IR_RAW_SAMPLE_TIME;
		reg_ir_raw_wl          = MIS_IR_RAW_WL;
		reg_ir_raw_deb         = MIS_IR_RAW_DEB;
		reg_ir_ctrl_rc6        = MIS_DUMMY;
	}
