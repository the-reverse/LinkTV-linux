...
{                   
    ...
    mcp_warning("Init MCP failed - invalid CPU\n");        
...
void _DumpDescriptor(mcp_desc* d)
{
    ...
    mcp_debug("do=%08lx\n",d->data_out);
    mcp_debug("len=%08lx\n",d->length);
    mcp_debug("XXXXXXXX desc end XXXXXXXXXX\n\n");
}
...
void _mcp_dump_desc_buffer(void)
{
    ...
    mcp_debug("BASE=%08x, RP=%08x, WP=%08x, LIMIT=%08x\n",
            GET_MCP_BASE(), 
            GET_MCP_RDPTR(), 
            GET_MCP_WRPTR(), 
            GET_MCP_LIMIT());    
    ...
    )
{
    ...
    switch(MARS_MCP_MODE(p_desc->flags))
    {
    case MCP_ALGO_DES:
...
static struct device_driver mcp_drv = 
{
