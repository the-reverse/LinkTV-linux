...
static int venus_rpc_suspend(struct device *dev, pm_message_t state, u32 level) {
	if (is_jupiter_cpu()) {
		if (level == SUSPEND_POWER_DOWN) {
			// Disable the interrupt from system to audio & video
			writel(0x6, (void *)0xb801a108);
//			prom_printf("venus_rpc_suspend\n");
		}
	}
	return 0;
}

static int venus_rpc_resume(struct device *dev, u32 level) {
	if (is_jupiter_cpu()) {
		if (level == RESUME_POWER_ON) {
			// Enable the interrupt from system to audio & video
			writel(0x7, (void *)0xb801a108);
//			prom_printf("venus_rpc_resume\n");
		}
	}
	return 0;
}
...
static struct device_driver venus_rpc_driver = {
	.name       = "Venus_RPC",
	.bus        = &platform_bus_type,
	.suspend    = venus_rpc_suspend,
	.resume     = venus_rpc_resume,
};
...
void RPC_cleanup_module(void)
{
    if (venus_rpc_devs != 0) {
        if (venus_rpc_devs->dev.driver != 0)
    	    driver_unregister(&venus_rpc_driver);
...
//      PDEBUG("   ***ptr[%d] = 0x%8x\n", i, (int)ptr);
    ...
    if ((result = driver_register(&venus_rpc_driver)) != 0) {
