...
static mcp_buff* __alloc_mcpb(unsigned int len)
{
    ...
    if (len >= MAX_MCPB_LEN )
    {
        p_buff = (mcp_buff*) __get_free_pages(GFP_KERNEL, get_order(len));
        
#ifdef USE_DVR_MALLOC                                           
        if (!p_buff) 
        {
             p_buff = dvr_malloc(len);                                   
             flag = MCPB_FLAG_DVR_MALLOC;
        }
#endif
    }
    else    
        p_buff = (mcp_buff*) kmalloc(len, GFP_KERNEL);    
...
void free_mcpb(mcp_buff* mcpb)
{
    ...
    {        
        ...
        {
#ifdef USE_DVR_MALLOC                    
            if ((mcpb->flag & MCPB_FLAG_DVR_MALLOC))
                dvr_free(mcpb);
            else
#endif
