...
int cdfs_copy_from_cdXA(struct super_block * sb, struct inode * inode, unsigned int start, unsigned int stop, char * buf)
{
	...
	&&((this_cd->bad_sector)<=((unsigned long long)lba))
	&&(((unsigned long long)lba)<(this_cd->bad_sector+70)))
		bad_sector_no_read=1;
      if(bad_sector_no_read)
      {
	      	status=-5;
		PRINT("<vcd module> Bad sector %d in VCD fail!!! status=%d bad_sector=%lld\n", lba, status,this_cd->bad_sector);
		return status;
      }
      else
      {
	  PRINT("<vcd module> Reading sector %d in VCD ST \n",lba);
	  status = (DVD_disc)?cdfs_read_iso_data(sb, lba, this_cd->cache):cdfs_read_raw_frame2(sb, lba, this_cd->cache,p->xa_data_size);
      }
