...
void ptpfs_set_inode_info(struct inode *ino, struct ptp_object_info *object)
{
	...
	printk("storage_id = %d\n",object->storage_id);
	printk("object_format=0x%x\t\tPTP_OFC_EXIF_JPEG=0x%x\n",object->object_format,PTP_OFC_EXIF_JPEG);
	//printk("protection_status=%d\n",object->protection_status);
	printk("object_compressed_size= %d Bytes = %d kBytes\n",object->object_compressed_size,object->object_compressed_size/1024);
	printk("thumb_format=0x%x\t\tPTP_OFC_JFIF=0x%x\n",object->thumb_format,PTP_OFC_JFIF);
	printk("thumb_compressed_size= %d = %d kBytes\n",object->thumb_compressed_size,object->thumb_compressed_size/1024);
	printk("thumb_pix_width=%d\t\t",object->thumb_pix_width);
	printk("thumb_pix_height=%d\n",object->thumb_pix_height);
	printk("image_pix_width=%d\t\t",object->image_pix_width);
	printk("image_pix_height=%d\t",object->image_pix_height);
	printk("image_bit_depth=%d\n",object->image_bit_depth);
	printk("parent_object=%d\t",object->parent_object);
	printk("association_type=0x%x\t",object->association_type);
	printk("association_desc=%d\n",object->association_desc);
...
checkagain2:
        ...
        {
		//printk("<ptp module> %s wait 0.1s pass=%d inode=0x%p\n",__func__,passport,ino);
                up(&ptp_passport_mutex);
                msleep(1000);
                goto checkagain2;
        }
...
{
    //printk(KERN_INFO "%s\n",  __FUNCTION__);
    printk("<ptp module> umount ptp device ST close_type=%d\n",PTPFSSB(sb)->usb_device->close_type);
    if(((PTPFSSB(sb)->usb_device->close_type)<0)||((PTPFSSB(sb)->usb_device->close_type)>2))
    {
    printk("<ptp module> Illegal close type, close_type=%d\n",PTPFSSB(sb)->usb_device->close_type);
    	return;
    }
...
//static int ptpfs_statfs(struct super_block *sb, struct statfs *buf)
static int get_ptpfs_statfs(struct super_block *sb)
{
	//printk(KERN_INFO "%s\n",  __FUNCTION__);
    struct ptp_storage_ids storageids;
    int x;
    printk("<ptp module> Read statfs data\n");
    
    PTPFSSB(sb)->kstatfs_temp.f_type = PTPFS_MAGIC;
	...
	{
        printk(KERN_INFO "Error getting storage ids 1\n");
        storageids.n = 0;
    ...
    for (x = 0; x < storageids.n; x++)
	{
        ...
        if (ptp_getstorageinfo(PTPFSSB(sb), storageids.storage[x],&storageinfo)!=PTP_RC_OK)
	        ...
	        {
    	    PTPFSSB(sb)->kstatfs_temp.f_blocks += storageinfo.max_capability;
	    PTPFSSB(sb)->kstatfs_temp.f_bfree  += storageinfo.free_space_in_bytes;
...
static long ptp_fill_super(struct super_block *sb, void *data, int silent)
{
    ...
    int x;
    __u16 ret;
	//printk("<ptp module> mount ptp device ST wait 2 sec\n");
	//msleep(1000);
	//msleep(1000);
    ...
    printk("<ptp module> mount ptp device SP and OK 0313\n\n\n");
    #ifdef DAVID_DEBUG
    printk("<ptp module> mount ptp device SP and OK time=%lu\n",get_seconds());
    printk("<ptp module> manufacturer=%s\n",PTPFSSB(sb)->deviceinfo->manufacturer);
    printk("<ptp module> model=%s\n",PTPFSSB(sb)->deviceinfo->model);
    printk("<ptp module> device_version=%s\n",PTPFSSB(sb)->deviceinfo->device_version);
    printk("<ptp module> serial_number=%s\n\n\n",PTPFSSB(sb)->deviceinfo->serial_number);
    #endif
    return 0;			
...
static int init_ptp_driver(void)
{
	int driver_result;
	int fs_result;
	printk("<ptp module> insert ptp module ST G 0313\n");
	#ifdef DAVID_DEBUG
    	printk("<ptp module> time=%lu\n",get_seconds());
	printk("<ptp module> DAVID_DEBUG Mode\n");
	printk("<ptp module> DAVID_DEBUG Mode\n");
	printk("<ptp module> DAVID_DEBUG Mode\n");
	printk("<ptp module> DAVID_DEBUG Mode MAX_SEG_SIZE=4096*%d\n",MAX_SEG_SIZE/4096);
	printk("<ptp module> DAVID_DEBUG Mode MAX_SEG_SIZE=4096*%d\n",MAX_SEG_SIZE/4096);
	printk("<ptp module> DAVID_DEBUG Mode MAX_SEG_SIZE=4096*%d\n",MAX_SEG_SIZE/4096);
	#endif
