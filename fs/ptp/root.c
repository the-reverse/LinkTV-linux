...
checkagain4:
	...
	up(&ptp_passport_mutex);
		...
		default:
			...
			{
				printk(KERN_INFO "Error getting storage ids 4\n");
				storageids.n = 0;
			...
			for (x = 0; x < storageids.n; x++)
			{
				...
				{
					printk(KERN_INFO "Error getting storage info 5\n");
				}
				else
				{
...
//    printk(KERN_INFO "%s\n",  __FUNCTION__);
	...
	{
        printk(KERN_INFO "Error getting storage ids 2\n");
        storageids.n = 0;
        storageids.storage = NULL;
	}
    ...
    for (x = 0; x < storageids.n; x++)
	{
        ...
        if (ptp_getstorageinfo(PTPFSSB(dir->i_sb), storageids.storage[x],&storageinfo)!=PTP_RC_OK)
 		{
            printk(KERN_INFO "Error getting storage info 3\n");
 		}
        else
        	{
            get_root_dir_name(&storageinfo,fsname,typeCount);
