...
int ptp_io_read(struct ptpfs_sb_info *sb, unsigned char *bytes, unsigned int size)
{
	...
	retval = usb_bulk_msg ( sb->usb_device->udev,pipe , bytes , size , &count , 100*HZ );
...
static __u16 ptp_usb_sendreq(struct ptpfs_sb_info *sb, struct ptp_container* req)
{
	...
	if(sb->read_condition == 1)	
	{
		...
		{	
			...
			{
				printk("5==== release function ! kmalloc error! ====\n");
			...
			if (ret < 0)
			{
				printk("6==== release function ! ptp_io_read error ! ====\n");
				ptp_free_data_buffer(buf);
				return PTP_ERROR_IO;
			}
			if (x == buf->num_seg-1) 
...
__u16 ptp_getdeviceinfo(struct ptpfs_sb_info *sb, struct ptp_device_info* deviceinfo)
{
    ...
    printk("<ptp module> %s:%s(%d) ret=0x%x\n",__FILE__,__func__,__LINE__,ret);
    if(ret!=PTP_RC_OK)
    {
    	ret=ptp_transaction(sb, &ptp, PTP_DP_GETDATA, 0, &data);
    	printk("<ptp module> %s:%s(%d) ret=0x%x\n",__FILE__,__func__,__LINE__,ret);
...
__u16 ptp_getstorageids(struct ptpfs_sb_info *sb, struct ptp_storage_ids* storageids)
{
    ...
    ptp_free_data_buffer(&data);
    #ifdef DAVID_DEBUG
    printk("<ptp module> %s:%s(%d) n=%u\n",__FILE__,__func__,__LINE__,storageids->n);
    printk("<ptp module> %s:%s(%d) storage=%u\n",__FILE__,__func__,__LINE__,*(storageids->storage));
    #endif
    return ret;
}
                            ...
                            struct ptp_object_handles* objecthandles)
{
    ...
    ptp_free_data_buffer(&data);
    #ifdef DAVID_DEBUG
    printk("<ptp module> %s:%s(%d) n=%u\n",__FILE__,__func__,__LINE__,objecthandles->n);
    printk("<ptp module> %s:%s(%d) storage=%u\n",__FILE__,__func__,__LINE__,*(objecthandles->handles));
    #endif
    return ret;
}
