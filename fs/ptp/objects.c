...
printk("\n<ptp module> %s do ptp_getobjectinfo %d times inode=0x%p\n",__func__,objects.n,inode);
    tmp=jiffies;
	    ...
	    //if((x%50)==0)
	    if(((x<50)&&((x%10)==0))||((x%50)==0))
	    {
		printk("<ptp module> %s do ptp_getobjectinfo %d times inode=0x%p x=%d\n",__func__,objects.n,inode,x);
		if(objects.n>=100)
			printk("<ptp module> Reading time is %lu seconds\n",(jiffies-tmp)/100);
	    }
...
checkagain2:
        ...
        {
		printk("<ptp module> %s wait 0.1s pass=%d\n",__func__,passport);
                up(&ptp_passport_mutex);
                msleep(1000);
                goto checkagain2;
        }
...
//	printk("offset_read start : %d, end : %d\n", start, end);
	for (x = start; x <= end; x++ ){			
		...
		if (buf->blocks[r].block == NULL){
			printk("1==== release function ! kmalloc error! ====\n");			
			return 0;
		}
		ret=ptp_io_read(sb_info,buf->blocks[r].block,MAX_SEG_SIZE);
		if (ret < 0){
			printk("2==== release function ! ptp_io_read error ! ====\n");
			return 0;
		}
...
delay_release:
	if(no_free==1)
		cnt++;
	cnt2++;
	//if((passport==0)||(passport==5))
	//if((passport==0)||(passport==5)||(passport==13)||(no_free==1))
	if((passport==5)||(passport==13)||(no_free==1))
	{
		//printk("<ptp module> %s wait 0.1s pass=%d no_free=%d cnt=%d cnt2=%d\n",__func__,passport,no_free,cnt,cnt2);
		if((no_free==1)&&(cnt>10))
			goto ok;
		if(cnt2==15)
			goto ok;
...
ok:
	printk("<ptp module> %s release inode=0x%p filp=0x%p pass=%d\n",__func__,ino,filp,passport);
	down(&ptp_passport_mutex);
	passport=PASSPORT_FREE;
	up(&ptp_passport_mutex);
	...
	if (data)
	{
		...
		{	
			...
			{				
				...
				if (data->blocks[1].block == NULL)
				{
					printk("3==== release function ! kmalloc error! ====\n");
					ptp_free_data_buffer(data);
					return PTP_ERROR_IO;
				}
				...
				if (ret < 0)
				{
					printk("4==== release function ! ptp_io_read error ! ====\n");
					ptp_free_data_buffer(data);
					return PTP_ERROR_IO;
				}
