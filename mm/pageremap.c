...
void list_all_memory_allocation(void);
...
retry:
		...
		if (order > page->private) {
			struct page *mypage = dvr_reserved_start;
			int aaa;
			printk(" ### REQ: %d GOT: %d \n", order, page->private);
			/* 
			 * There might be a race condition when a page allocated from pcp list
			 * while we do the remap_DVR_region(). In this case, we do not know this
			 * page is freed or just be allocated from pcp list. So, we do remapping
			 * again.
			 */
			yield();
			goto retry;
			for (aaa = 0; aaa < dvr_reserved_count; aaa++) {
				printk("^^^PAGE: %p private: %d count: %d mapcount: %d flags: %x mapping: %p\n", 
					mypage, mypage->private, page_count(mypage), page_mapcount(mypage), 
					mypage->flags, mypage->mapping);
				mypage++;
			}
