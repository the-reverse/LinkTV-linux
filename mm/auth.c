...
//				printk("can't find task struct...\n");
		...
		case AUTH_IOCTINITHWSEM:
			...
			outl(0x000000c, VENUS_SB2_INV_INTSTAT);
			break;
		...
		case AUTH_IOCTGETHWSEM:
			{
...
#ifdef	CONFIG_REALTEK_USE_HWSEM_AS_SENTINEL
				volatile unsigned int *reset_ptr = JUPITER_SW_MUTEX_RESET;
				...
				while (1) {
					...
					// check the sw semaphore...
					if (*reset_ptr == 1) {
						// get the sw semaphore...
						*reset_ptr = 0;

						// put the hw semaphore...
						*ptr = 0;
						local_irq_enable();
						break;
					}
