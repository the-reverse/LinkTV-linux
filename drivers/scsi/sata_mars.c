...
unsigned int SCR2_VOL; //300:no speed restrictions; 310: limit on G1; 320: limit on G2
...
void mars_ata_dma_reset(struct ata_port *ap)
{
    ...
    /*Mask SB1 sata*/
    //printk("%s(%d)phy_bank:%d\n",__func__,__LINE__,phy_bank);
...
static void init_mdio (u8 bank)
{
    unsigned int reginfo;
    if(is_jupiter_cpu()){
        printk("jupiter mdio setting enable\n");
        reginfo=readl((void __iomem *)0xb801a204) & 0x00f00000;

        mdio_setting(0x00, 0x1066,0);    //0xb8012be8=0x0066 00 01
        mdio_setting(0x01, 0x0003,0);    //0xb8012be8=0x0003 01 01
        mdio_setting(0x02, 0x2d18,0);    //0xb8012be8=0x2d18 02 01
        mdio_setting(0x03, 0x6d09,0);    //0xb8012be8=0x6d09 03 01
        mdio_setting(0x04, 0x51C7,0);    //0xb8012be8=0x51C7 04 01
        mdio_setting(0x05, 0x0b5a,0);    //0xb8012be8=0x0b5a 05 01
        mdio_setting(0x06, 0xe468,0);    //0xb8012be8=0xe468 06 01
        mdio_setting(0x07, 0x31f8,0);    //0xb8012be8=0x31fe 07 01
        mdio_setting(0x08, 0x18da,0);    //0xb8012be8=0x18da 08 01

        if(reginfo<0x00c00000)
            mdio_setting(0x09, 0x9310,0);    //0xb8012be8=0x9310 09 01    //for 1185 (A/B)

        mdio_setting(0x0a, 0x03a8,0);    //0xb8012be8=0x03a8 0a 01
        mdio_setting(0x0b, 0x0111,0);    //0xb8012be8=0x0111 0b 01
        mdio_setting(0x0c, 0x3ff9,0);    //0xb8012be8=0x3ff9 0c 01
        mdio_setting(0x0d, 0xfa60,0);    //0xb8012be8=0xfa60 0d 01
        mdio_setting(0x0f, 0x0000,0);    //0xb8012be8=0x0000 0f 01
        mdio_setting(0x1e, 0xc4ea,0);    //0xb8012be8=0xc4ea 1e 01

        if(reginfo<0x00c00000){
            mdio_setting(0x09, 0x9300,0);    //0xb8012be8=0x9300 09 01 //OOBs autoK   //for 1185 (A/B)
            mdio_setting(0x09, 0x9100,0);    //0xb8012be8=0x9100 09 01 //OOBs autoK   //for 1185 (A/B)
            mdio_setting(0x09, 0x9300,0);    //0xb8012be8=0x9300 09 01 //OOBs autoK   //for 1185 (A/B)
        }else{
            //mdio_setting(0x09, 0x9302,0);    //0xb8012be8=0x9300 09 01 //OOBs autoK
            //mdio_setting(0x09, 0x9102,0);    //0xb8012be8=0x9100 09 01 //OOBs autoK
            //mdio_setting(0x09, 0x9302,0);    //0xb8012be8=0x9300 09 01 //OOBs autoK
            mdio_setting(0x09, 0x9304,0);    //0xb8012be8=0x9300 09 01 //OOBs autoK
        }

        mdelay(1);
        mdio_setting(0x1c, 0xff00,0);    //0xb8012be8=0xff00 1c 01 //Tx Rx swap

        mdelay(1);
        mdio_setting(0x0a, 0x0ba8,0);    //0xb8012be8=0x0ba8 0a 01  //soft reset
        mdelay(1);
        mdio_setting(0x0a, 0x03a8,0);    //0xb8012be8=0x03a8 0a 01  //soft reset

        mdelay(1);
        writel(0x3fdef1c7,(void __iomem *)CRT_SOFT_RESET1);   //0xb8000000=0x3fdef1c7 //CRT SATA PHY reset

        mdelay(1);
        writel(0x3fdef1d7,(void __iomem *)CRT_SOFT_RESET1);   //0xb8000000=0x3fdef1d7 //CRT SATA PHY reset

    }else{
        printk("mars mdio setting enable\n");
...
extra_try:
        ...
        printk("%s(%d)ap->port_no:%d\n",__func__,__LINE__,ap->port_no);
        ap->ops->dma_reset(ap);
    }
    else
    {
...
reset_taskfile:
    ...
    printk("%s(%d)tf status:%x\n",__func__,__LINE__,ata_chk_status(ap));
...
static int mars_init_sata (struct device *dev)
{
    ...
    if(is_jupiter_cpu()){
        reginfo=readl((void __iomem *)0xb801a204) & 0x00f00000;
        if(reginfo<0x00c00000){
            SCR2_VOL=0x310;//310:limit on G1; 320: limit on G2
            printk("JUPITER(Ver.A/B): one port supported\n");
        }else{
            SCR2_VOL=0x300;//300:no speed restrictions;
            printk("JUPITER(Ver.C/D): one port supported\n");
        }

        phy_bank_cnt=1;
    }else{
        printk("MARS: two ports supported.\n");
        phy_bank_cnt=SATA_PORT_COUNT;
    }
 ...
 *********************************************************/
#define NULL_SPACE 4
#define SERIAL_NUM_SIZE 20+NULL_SPACE
static ssize_t
show_serieal_field(struct bus_type *bus, char *buf)
{
    struct ata_host_set *host_set = dev_get_drvdata(dev_addr);
    struct ata_port *ap;
    struct ata_device *adev;
    u8 i;
    u8 *c1;
    static char serial_num[SERIAL_NUM_SIZE];
    ...
    for(i=0; i<10; i++){
        c1=(u8 *)&(adev->id[10+i]);
        //printk("%c%c",*(c1+1),*(c1) );
        serial_num[i*2+0]=*(c1+1);
        serial_num[i*2+1]=*(c1+0);
    }
    //printk("\n");
...
static ssize_t
store_serieal_field(struct bus_type *bus, const char *buf, size_t count)
{
    //struct ata_host_set *host_set = dev_get_drvdata(dev_addr);
    //struct ata_port *ap;
    //struct ata_device *adev;
...
static BUS_ATTR(serieal, S_IRUGO | S_IWUSR, show_serieal_field, store_serieal_field);

static int __init mars_init(void)
{
    int error;

    printk(KERN_INFO"sata driver initial...2011/05/20 17:10\n");
    ...
    if (error)
        ...
        bus_create_file(&sata_bus_type, &bus_attr_serieal);
...
err_device_fail:
    ...
    bus_remove_file(&sata_bus_type, &bus_attr_serieal);
