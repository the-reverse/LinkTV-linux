	...
	)
{
	...
	case LED_SCAN_BLINK:
		if( (((check_fwstate(pmlmepriv, _FW_LINKED)== _TRUE)&&(pmlmepriv->fw_state & WIFI_STATION_STATE) ) ||
			((pmlmepriv->fw_state & WIFI_ADHOC_STATE) || (pmlmepriv->fw_state & WIFI_ADHOC_MASTER_STATE)) ) && // Linked.
			(check_fwstate(pmlmepriv, _FW_UNDER_SURVEY)== _FALSE) && // Not in scan stage.
	...
	case LED_BLINK_StartToBlink:
		if( (check_fwstate(pmlmepriv, _FW_LINKED)== _TRUE)&&(pmlmepriv->fw_state & WIFI_STATION_STATE) )// Linked.
		{
			bStopBlinking = _TRUE;
		}
	...
	IN	PADAPTER			Adapter)
{
	...
	if(Adapter->bfirst_init)
	{
		DBG_8192C("gen_RefreshLedState first init\n");
		return;
	}
	...
	if(pwrctrlpriv->rfoff_reason == RF_CHANGE_BY_IPS )
	{
		switch(pledpriv->LedStrategy)
		{
			...
			case SW_LED_MODE8:
			case SW_LED_MODE9:
				pledpriv->LedControlHandler(Adapter, LED_CTL_NO_LINK);
				break;					
	...
	else		// SW/HW radio off
	{
		...
		{
			case SW_LED_MODE9:
				pledpriv->LedControlHandler(Adapter, LED_CTL_POWER_OFF);
				break;				
