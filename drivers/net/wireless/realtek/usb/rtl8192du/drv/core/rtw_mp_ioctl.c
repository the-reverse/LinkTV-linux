...
_func_enter_;
	...
	write_bbreg(Adapter, offset, bMaskDWord, value);
...
_func_enter_;
	...
	_irqlevel_changed_(&oldirql, LOWER);
	value = read_bbreg(Adapter, offset, bMaskDWord);
	_irqlevel_changed_(&oldirql, RAISE);
...
_func_enter_;
	...
	_irqlevel_changed_(&oldirql, LOWER);
 	write_rfreg(Adapter, path, offset, bMaskDWord, value);
	_irqlevel_changed_(&oldirql, RAISE);
...
_func_enter_;
	...
	path = (u8)pbbreg->path;
	if (path > MAX_RF_PATH_NUMS) // 1T2R  path_a /path_b
		return NDIS_STATUS_NOT_ACCEPTED;

	offset = (u8)pbbreg->offset;

	_irqlevel_changed_(&oldirql, LOWER);
	value = read_rfreg(Adapter, path, offset, bMaskDWord);
	_irqlevel_changed_(&oldirql, RAISE);
...
#ifdef CONFIG_RTL8192C
	val8 = read8(Adapter, MSR) & 0xFC; // 0x0102
	val8 |= WIFI_FW_ADHOC_STATE;
	write8(Adapter, MSR, val8); // Link in ad hoc network
#endif

#ifndef CONFIG_RTL8192C
	write8(Adapter, MSR, 1); // Link in ad hoc network
...
_func_enter_;
	...
	_irqlevel_changed_(&oldirql, LOWER);
	GetThermalMeter(Adapter, (u32*)poid_par_priv->information_buf);
	_irqlevel_changed_(&oldirql, RAISE);
...
_func_enter_;
	...
	if ((addr + cnts) > EFUSE_AVAILABLE_MAX_SIZE) {
...
_func_enter_;
	...
	*poid_par_priv->bytes_rw = 0;
	...
	if (poid_par_priv->information_buf_len < EFUSE_MAP_SIZE)
		return NDIS_STATUS_INVALID_LENGTH;
	...
	if (poid_par_priv->type_of_oid == QUERY_OID)
	{
		...
		if (efuse_map_read(Adapter, 0, EFUSE_MAP_SIZE, data) == _SUCCESS)
			*poid_par_priv->bytes_rw = EFUSE_MAP_SIZE;
		else {
		...
		if (efuse_map_write(Adapter, 0, EFUSE_MAP_SIZE, data) == _SUCCESS)
			*poid_par_priv->bytes_rw = EFUSE_MAP_SIZE;
		else {
			RT_TRACE(_module_mp_, _drv_err_,
