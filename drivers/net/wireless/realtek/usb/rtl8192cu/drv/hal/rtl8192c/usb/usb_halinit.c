...
//=============== Init MAC ======================
	...
	pollingCount = 0;
	write8(padapter, REG_APSD_CTRL, (read8(padapter, REG_APSD_CTRL) & ~BIT6));
	do{
		pollingCount++;	
	}while((pollingCount<200) && (read8(padapter, REG_APSD_CTRL)&BIT7)); //polling until BIT7 is 0. by tynli
	...
	)
{
	...
	if (!SUPPORT_HW_RADIO_DETECT(pHalData))
		return;
	...
	)
{
	...
	//
	rtl8192c_ReadBluetoothCoexistInfo(Adapter, PROMContent, AutoloadFail);
#endif
