...
//	range->max_r_time;	/* Maximal retry lifetime */
	...
	for (i = 0, val = 0; i < 32; i++) {
		...
		if(pmlmeext->channel_set[i] != 0)
		{
			...
			range->freq[val].m = ch2freq(pmlmeext->channel_set[i]) * 100000;
			range->freq[val].e = 1;
			val++;
		}
                            ...
                            union iwreq_data *wrqu, char *extra)
{
	...
	path = *(u32*)extra;
	addr = *((u32*)extra + 1);
	data32 = padapter->HalFunc.read_rfreg(padapter, path, addr, 0xFFFFFFFF);
                            ...
                            union iwreq_data *wrqu, char *extra)
{
	...
	data32 = *((u32*)extra + 2);
//	printk("%s: path=%d addr=0x%02x data=0x%05x\n", __func__, path, addr, data32);
	padapter->HalFunc.write_rfreg(padapter, path, addr, 0xFFFFFFFF, data32);
...
static int rtw_wx_set_countrycode(struct net_device *dev,
                               struct iw_request_info *info,
                               union iwreq_data *wrqu, char *extra)
{
	_adapter *padapter = netdev_priv(dev);
...
static int rtw_wps_get_peer_ifaddr(struct net_device *dev,
                               struct iw_request_info *info,
                               union iwreq_data *wrqu, char *extra)
{
	int ret = 0;
	...
	_list	*plist, *phead;
	u8	wps_ssid_len;
	s8	wps_ssid[WPS_MAX_DEVICE_NAME_LEN];
	u8	wps_bssid[ETH_ALEN] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
	u8	wps_ie[512];
	uint	wps_ielen;
	struct wlan_network *pnetwork = NULL;
	_adapter	*padapter = netdev_priv(dev);
	...
	_memset(wps_ssid, 0, WPS_MAX_DEVICE_NAME_LEN);

	wps_ssid_len = wrqu->data.length -1;
	//printk("%s(): wps_ssid_len = %d \n",__FUNCTION__,wps_ssid_len);

	if(copy_from_user(wps_ssid, pdata->pointer, wps_ssid_len))
	{
		ret= -EINVAL;
		printk("%s(): copy_from_user fail \n",__FUNCTION__);
		goto exit;
	}
	//printk("%s(): wps_ssid = %s \n",__FUNCTION__,wps_ssid);
	...
	while(1)
	{
		...
		if( (_memcmp(wps_ssid, pnetwork->network.Ssid.Ssid, wps_ssid_len)) == _TRUE)//SSID match
		{
			if(get_wps_ie(pnetwork->network.IEs, pnetwork->network.IELength, wps_ie, &wps_ielen)==_TRUE)
			{
				_memcpy(wps_bssid, pnetwork->network.MacAddress, ETH_ALEN);
				//printk("BSSID:" MACSTR "\n", MAC2STR(wps_bssid));
				break;
			}
		}
	...
	sprintf( extra, "\n%.2X%.2X%.2X%.2X%.2X%.2X", wps_bssid[0],wps_bssid[1],wps_bssid[2],wps_bssid[3],wps_bssid[4],wps_bssid[5]);
...
{
	u8 *p;
	u8 val8, cur_channel, cur_bwmode, cur_ch_offset, bxmitok=_FALSE;
	u16 bcn_interval;
	u32	acparm;	
	int	ie_len, retry=0;	
	struct registry_priv	 *pregpriv = &padapter->registrypriv;
...
//	Need to set the correct ioctl code.
	...
	},
	{
		...
		IW_PRIV_TYPE_CHAR | WPS_MAX_DEVICE_NAME_LEN, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_FIXED | IFNAMSIZ , "wps_get_peerifa"
	},
	{
...
#ifdef RTK_DMP_PLATFORM
	rtw_wx_set_countrycode, //Set Channel depend on the country code, 0xa
	rtw_wx_priv_null,			//	0xb
	rtw_wx_priv_null,			//	0xc	
#else
#ifdef CONFIG_PLATFORM_MT53XX
