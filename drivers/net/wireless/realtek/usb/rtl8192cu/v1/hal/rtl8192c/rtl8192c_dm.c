...
void dm_DynamicTxPower(IN	PADAPTER	Adapter)
{
	...
	if( (pdmpriv->DynamicTxHighPowerLvl != pdmpriv->LastDTPLvl) )
	{
		...
		if((pmlmeext->cur_bwmode == HT_CHANNEL_WIDTH_20)||(pmlmeext->cur_ch_offset == HAL_PRIME_CHNL_OFFSET_DONT_CARE))
		{
		...
		else		
		{
			...
			if (pmlmeext->cur_ch_offset == HAL_PRIME_CHNL_OFFSET_LOWER)
			{
				PHY_SetTxPowerLevel8192C(Adapter, pHalData->CurrentChannel + 2);
			}
			else
			{
				PHY_SetTxPowerLevel8192C(Adapter, pHalData->CurrentChannel - 2);
            ...
            IN PADAPTER	Adapter)
{
	...
	int			Y, ele_C;
	char			OFDM_index[2], CCK_index, OFDM_index_old[2], CCK_index_old;
	int i = 0;
	BOOLEAN			is2T;
	...
	rtl8192c_PHY_APCalibrate(Adapter, (ThermalValue - pHalData->EEPROMThermalMeter));//notes:EEPROMThermalMeter is a fixed value from efuse/eeprom
...
void dm_InitDynamicBBPowerSaving(
	IN	PADAPTER	Adapter
	)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
	...
	)
{
	...
	if(pdmpriv->initialize == 0){
		pdmpriv->Reg874 = (PHY_QueryBBReg(pAdapter, rFPGA0_XCD_RFInterfaceSW, bMaskDWord)&0x1CC000)>>14;
		pdmpriv->RegC70 = (PHY_QueryBBReg(pAdapter, rOFDM0_AGCParameter1, bMaskDWord)&BIT3)>>3;
		pdmpriv->Reg85C = (PHY_QueryBBReg(pAdapter, rFPGA0_XCD_SwitchControl, bMaskDWord)&0xFF000000)>>24;
		pdmpriv->RegA74 = (PHY_QueryBBReg(pAdapter, 0xa74, bMaskDWord)&0xF000)>>12;
	...
	if(pPSTable->PreRFState != pPSTable->CurRFState)
	{
		...
		{
			PHY_SetBBReg(pAdapter, rFPGA0_XCD_RFInterfaceSW  , 0x1CC000, pdmpriv->Reg874); 
			PHY_SetBBReg(pAdapter, rOFDM0_AGCParameter1, BIT3, pdmpriv->RegC70); 
			PHY_SetBBReg(pAdapter, rFPGA0_XCD_SwitchControl, 0xFF000000, pdmpriv->Reg85C);
			PHY_SetBBReg(pAdapter, 0xa74, 0xF000, pdmpriv->RegA74); 
