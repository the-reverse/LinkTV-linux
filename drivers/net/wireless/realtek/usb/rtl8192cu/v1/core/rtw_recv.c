...
int	init_recv_priv(struct recv_priv *precvpriv, _adapter *padapter)
{
	...
	max_recvbuf_sz = precvpriv->max_recvbuf_sz;
	nr_recvbuf = precvpriv->nr_recvbuf;

	//init recv_buf
	_init_queue(&precvpriv->free_recv_buf_queue);

	precvpriv->pallocated_recv_buf = _malloc(nr_recvbuf *sizeof(struct recv_buf) + 4);
	if(precvpriv->pallocated_recv_buf==NULL){
		res= _FAIL;
		RT_TRACE(_module_rtl871x_recv_c_,_drv_err_,("alloc recv_buf fail!\n"));
		goto exit;
	}
	_memset(precvpriv->pallocated_recv_buf, 0, nr_recvbuf *sizeof(struct recv_buf) + 4);
	...
	precvpriv->free_recv_buf_queue_cnt = nr_recvbuf;
...
#ifdef CONFIG_PREALLOC_RECV_SKB
	{
		...
		nr_prealloc_recv_skb = nr_recvbuf * 2;
...
void free_recv_priv (struct recv_priv *precvpriv)
{
	...
	for(i=0; i < precvpriv->nr_recvbuf ; i++)
	{
	...
	if(precvpriv->pallocated_recv_buf)
		_mfree(precvpriv->pallocated_recv_buf, precvpriv->nr_recvbuf *sizeof(struct recv_buf) + 4);
...
int init_recvbuf(_adapter *padapter, struct recv_buf *precvbuf)
{
	...
	if(precvbuf->pbuf)
	{
		...
		precvbuf->pend = precvbuf->pdata + padapter->recvpriv.max_recvbuf_sz;
	}
...
static int recv_func(_adapter *padapter, void *pcontext)
{
	...
	padapter->HalFunc.process_phy_info(padapter, prframe);
