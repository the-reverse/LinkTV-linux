...
//#define REG_DBG(port)           printk("%s : reg=%8x\n", __FUNCTION__, port);        
#define REG_DBG(port)           
...
static inline void NdisRawReadPortUshort(u32 port, u16 *pval)
{
	...
	//*pval = inw(port);
	*pval = readw(RegAddr(port));
}

static inline void NdisRawReadPortUlong(u32 port, u32 *pval)
{
	//*pval = inl(port);
	*pval = readl(RegAddr(port));
}
