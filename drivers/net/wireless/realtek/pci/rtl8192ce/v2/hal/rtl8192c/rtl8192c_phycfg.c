	...
	)
{
	...
	if(t==0)
	{
		...
		//RTPRINT(FINIT, INIT_IQK, ("IQ Calibration for %s\n", (is2T ? "2T2R" : "1T1R")));
	 	...
	 	_PHY_SaveADDARegisters(pAdapter, ADDA_REG, pdmpriv->ADDA_backup,16);
	...
	if(t==0)
	{
		// Store 0xC04, 0xC08, 0x874 vale
	 	pdmpriv->RegC04 = PHY_QueryBBReg(pAdapter, 0xc04, bMaskDWord);
	 	pdmpriv->RegC08 = PHY_QueryBBReg(pAdapter, 0xc08, bMaskDWord);
	 	pdmpriv->Reg874 = PHY_QueryBBReg(pAdapter, 0x874, bMaskDWord);
	 	pdmpriv->RegB68 = PHY_QueryBBReg(pAdapter, 0xb68, bMaskDWord);
	 	pdmpriv->RegB6C = PHY_QueryBBReg(pAdapter, 0xb6c, bMaskDWord);
	 	pdmpriv->Reg870 = PHY_QueryBBReg(pAdapter, 0x870, bMaskDWord);
	 	pdmpriv->Reg860 = PHY_QueryBBReg(pAdapter, 0x860, bMaskDWord);
	 	pdmpriv->Reg864 = PHY_QueryBBReg(pAdapter, 0x864, bMaskDWord);
	}
	...
	PHY_SetBBReg(pAdapter, 0xc04, bMaskDWord, pdmpriv->RegC04);
	PHY_SetBBReg(pAdapter, 0x874, bMaskDWord, pdmpriv->Reg874);
	PHY_SetBBReg(pAdapter, 0xc08, bMaskDWord, pdmpriv->RegC08);
	...
	if(t!=0)
	{
		...
		_PHY_ReloadADDARegisters(pAdapter, ADDA_REG, pdmpriv->ADDA_backup, 16);

		// Reload MAC parameters
		_PHY_ReloadMACRegisters(pAdapter, IQK_MAC_REG, pdmpriv->IQK_MAC_backup);

		PHY_SetBBReg(pAdapter, 0xb68, bMaskDWord, pdmpriv->RegB68);
		PHY_SetBBReg(pAdapter, 0xb6c, bMaskDWord, pdmpriv->RegB6C);
		PHY_SetBBReg(pAdapter, 0x870, bMaskDWord, pdmpriv->Reg870);
		PHY_SetBBReg(pAdapter, 0x860, bMaskDWord, pdmpriv->Reg860);
		PHY_SetBBReg(pAdapter, 0x864, bMaskDWord, pdmpriv->Reg864);
...
//						0x00, 0x10, 0x11};						
	...
	//save AFE default value
	_PHY_SaveADDARegisters(pAdapter, AFE_REG, AFE_backup, APK_AFE_REG_NUM);
	...
	//reload AFE default value
	_PHY_ReloadADDARegisters(pAdapter, AFE_REG, AFE_backup, APK_AFE_REG_NUM);	
	...
	)
{
	...
	if(bReCovery)
	{
		_PHY_ReloadADDARegisters(pAdapter, IQK_BB_REG, pdmpriv->IQK_BB_backup, 10);
		return;
	}
	...
	_PHY_SaveADDARegisters(pAdapter, IQK_BB_REG, pdmpriv->IQK_BB_backup, 10);
}
