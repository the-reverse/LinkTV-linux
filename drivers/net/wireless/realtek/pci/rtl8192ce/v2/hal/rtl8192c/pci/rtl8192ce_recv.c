...
void rtl8192ce_update_recvframe_attrib_from_recvstat(union recv_frame *precvframe, struct recv_stat *prxstat)
{
	...
	if(pattrib->crc_err)
		DBG_8192C("##### Receive crc error pakcet !!!!! #####\n");
	...
	if(drvinfo_sz && physt)
	{
		pphy_info = (struct phy_stat *)(precvframe->u.hdr.rx_data - drvinfo_sz);
//printk("%s(): %p %x %p\n",__FUNCTION__,precvframe->u.hdr.rx_data, drvinfo_sz, pphy_info);
