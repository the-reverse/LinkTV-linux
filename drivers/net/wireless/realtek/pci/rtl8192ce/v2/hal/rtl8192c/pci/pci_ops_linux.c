...
static void rtl8192ce_rx_mpdu(_adapter *padapter)
{
	...
	while (count--)
	{
		...
		else
		{
			...
			//printk("%s(): idx = %d\n",__FUNCTION__,precvpriv->rx_ring[rx_queue_idx].idx);
			...
			if(pkt_copy)
			{
				...
				//printk("(1)pkt_copy = %p, data = %p size = %d\n",pkt_copy,pkt_copy->data, alloc_sz);
				...
				//printk("(2)pkt_copy = %p, data = %p(%p)\n",pkt_copy,pkt_copy->data, precvframe->u.hdr.rx_data);
			}
			else
			{	
...
//printk("%s(): %p %x %x\n",__FUNCTION__,precvframe->u.hdr.rx_data, drvinfo_sz,shift_sz);
...
static void pci_write8(struct intf_hdl *pintfhdl, u32 addr, u8 val)
{
	...
	read8(pdvobjpriv->padapter, addr);
...
{	
	...
	read16(pdvobjpriv->padapter, addr);
...
static void pci_write32(struct intf_hdl *pintfhdl, u32 addr, u32 val)
{
	...
	read32(pdvobjpriv->padapter, addr);
