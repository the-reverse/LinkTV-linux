...
#define REALTEK_DMP_PLATFORM        // kevin add 20090820
...
#ifdef REALTEK_DMP_PLATFORM		
		ctl &= ~PCI_EXP_DEVCTL_READRQ;      // Force Read Request Size to 128   
		printk("%s : Force Read Request Size to 128 bits\n",__FUNCTION__);
#endif				
			   ...
			   u32 start, u32 end)
{
	...
	for (cur = start; end - cur != 0; cur++) {
        ...
        //printk("skb %d (%p), head = %p, data = %p, tail=%p, end=%p\n",
          //      cur, skb, skb->head, skb->data, skb->tail, skb->end);		
		tp->Rx_skbuff[i] = skb;
	}
	return cur - start;
}
...
static irqreturn_t rtl8169_interrupt(int irq, void *dev_instance, struct pt_regs *regs)
{
	...
	do {
		...
		RTL_W16(IntrMask, tp->intr_mask & ~tp->intr_event);
		tp->intr_mask = ~tp->intr_event;
