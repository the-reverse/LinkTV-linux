...
//		        cp_rx_frag(cp, rx_tail, skb, status, len); // cyhuang add 
		...
		//printk("%s:%d skb->data=0x%x , skb->tail=0x%x \n",
		//	__FUNCTION__,__LINE__,skb->data, skb->tail);//cy test
                if (status & MulAddrRx) 
...
static int cp_start_xmit (struct sk_buff *skb, struct net_device *dev)
{
	...
	u32 eor;
#if 0
			printk("cp_start_xmit : skb->data=0x%x \n",skb->data);
			{
				u8 *p;
				int i;
				p = (u8 *)skb->data ;
...
static int cp_init_hw (struct cp_private *cp)
{
	...
	u32 *hwaddr;
        ...
        while (((cp->mii_if.mdio_read(dev , cp->mii_if.phy_id, 1)&0xffdb) != 0x7949) 
		&& (i<10)) {
...
//	cp->mii_if.mdio_write(dev , 1, 4, 0x05e1); 
	...
	else if (CHIP_REG == Jupiter_id) /* for fpga */
		cp->mii_if.mdio_write(dev , cp->mii_if.phy_id, 0, 0x1000);
	else	
		cp->mii_if.mdio_write(dev , cp->mii_if.phy_id, 0, 0x2100);
...
*/	
	...
	if (CHIP_REG == Jupiter_id) {
		//cpw32(Ethrnt_Ctrl, 0x48c3); //10/100
                cpw32(Ethrnt_Ctrl, 0x80e3);   //1G
		cpw32(Ethrnt_Ctrl1, 0x52080);
		cpw32(Ethrnt_Phase_Sel, 0x300);	
