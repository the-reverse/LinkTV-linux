...
/* chip id */
#define CHIP_REG \
        (*(volatile unsigned int*)(0xb801a200)&0x0000ffff)

#define CHIP_INFO \
	(*(volatile unsigned int*)(0xb801a204)&0xffff0000)
...
#define restart_module() \
            writel(readl(0xb8000000) & 0xffff9fff, 0xb8000000 ); \
 	    writel(readl(0xb800000c) & 0xfffffdff, 0xb800000c);    \
	    writel(readl(0xb8000000) | (~0xffff9fff), 0xb8000000);	  \
            writel(readl(0xb800000c) | (~0xfffffdff), 0xb800000c);  

#define init_phy 0x7849
...
enum CP_STATUS_REGS
{
	...
	LastFrag	= (1 << 28), /* Final segment of a packet */
        MulAddrRx       = (1 << 26), /* Multicast Address Rx */
        BrdAddrRx       = (1 << 24), /* Broadcast Address Rx */			
	TxError		= (1 << 23), /* Tx error summary */
	RxError		= (1 << 20), /* Rx error summary */
	RxCRC		= (1 << 18), /* Rx CRC error */
...
enum CP_THRESHOLD_REGS{
	...
	RINGSIZE_2	= 0x7f00,    /* 0xf00 :16 desc, 0x1f00: 32 desc, 0x3f00:64 desc, 0x7f00:128 desc */

	LOOPBACK	= (0x3 << 8),
	AcceptFlctl     = 0x40,      /* Accept flow control packets */  
...
#define CONFIG_R8139_NAPI

#ifdef  CONFIG_R8139_NAPI
#define rtl8139_rx_skb          netif_receive_skb
#define rtl8139_rx_hwaccel_skb  vlan_hwaccel_receive_skb
#else
#define rtl8139_rx_skb          netif_rx
#define rtl8139_rx_hwaccel_skb  vlan_hwaccel_rx       		 
#endif 
...
//#define CONFIG_RTL8139CP_TIMER_INTR_MITIGATION

#ifdef CONFIG_RTL8139CP_TIMER_INTR_MITIGATION

// using generate timer to mitigate the interrupt count

typedef struct {
    unsigned long   us;
    void (*handler)(unsigned long param);    
    unsigned long   param;
}rt_timer_t;               
     
     
static irqreturn_t cp_interrupt (int irq, void *dev_instance, struct pt_regs *regs);
void stop_cp_timer();
          
static void cp_timer(unsigned long param)
{
    struct net_device *dev = (struct net_device *) param;
    struct cp_private *cp = netdev_priv(dev);	    
                
    if (cp_interrupt(0, dev, NULL)==IRQ_NONE)
    {        
        stop_cp_timer();
...
static spinlock_t cp_timer_lock = SPIN_LOCK_UNLOCKED; 
extern int request_rt_timer(rt_timer_t* p_timer);
extern void release_rt_timer(rt_timer_t* p_timer);

int start_cp_timer(struct net_device *dev)
{
    int ret = 0;
    unsigned long flags;    
    
    spin_lock_irqsave(&cp_timer_lock, flags);
	
    if (!poll_enable)
    {   
        if (request_rt_timer(&cp_2nd_timer)==0)                         
        {   
            poll_enable = 1;
            cp_2nd_timer.param = dev;                        
        }                             
    }
    
    ret = (poll_enable) ? 0 : -1;
        
    spin_unlock_irqrestore(&cp_timer_lock, flags);
			      ...
			      struct cp_desc *desc)
{
	...
	if (cp->vlgrp && (desc->opts2 & RxVlanTagged)) {
	    rtl8139_rx_hwaccel_skb(skb, cp->vlgrp,
...
//            printk("error:RX FIFO overflow , status = 0x%x \n",status);
	...
	if (status & (TX_OK | TX_ERR | TX_EMPTY | SW_INT))
        ...
        budget = 64;
        cp_rx_poll(dev, &budget);                            
        cp_tx(cp);            
        
    #ifdef CONFIG_RTL8139CP_TIMER_INTR_MITIGATION
        if (irq)        
        {            
            if (start_cp_timer(dev)==0)    // enable cp timer
            {                
                cpw16_f(IMR, 0);                // stop all interrupts                        
            }                
        }                
    #endif             
...
static int cp_start_xmit (struct sk_buff *skb, struct net_device *dev)
{
	...
	unsigned entry;
	u32 eor;
#ifdef CONFIG_JUPITER_ETH //hcy workaround
 	if(CHIP_INFO == 0x00b00000 || CHIP_INFO == 0x00a00000)  
        if (((int)(skb->data) & 0x4) && ((skb->len%256) <=4 ))
                skb->len += (4-skb->len%256) ;
#endif
...
static void __cp_set_rx_mode (struct net_device *dev)
{
	...
	if (dev->flags & IFF_PROMISC) {
		...
		rx_mode =
		    ...
		    AcceptAllPhys | AcceptErr ;
		mc_filter[1] = mc_filter[0] = 0xffffffff;
	} else if ((dev->mc_count > multicast_filter_limit)
		   || (dev->flags & IFF_ALLMULTI)) {
		printk (KERN_NOTICE "%s: Accept all multicast.\n",
		        dev->name);
		/* Too many to filter perfectly -- accept all multicasts. */
		rx_mode = AcceptBroadcast | AcceptMulticast | AcceptMyPhys | AcceptErr ;
		mc_filter[1] = mc_filter[0] = 0xffffffff;
	} else {
		struct dev_mc_list *mclist;
		rx_mode = AcceptBroadcast | AcceptMyPhys | AcceptErr ; 
	        printk (KERN_NOTICE "\n %s: Set multicast.\n",
                        dev->name);
...
#ifdef CONFIG_JUPITER_ETH //hcy workaround
 	if(CHIP_INFO == 0x00c00000 ) {
		printk (KERN_NOTICE "%s:  all accept mode enabled in C revision of jupiter .\n", dev->name);

		rx_mode = AcceptAll | AcceptFlctl;
		mc_filter[1] = mc_filter[0] = 0xffffffff;
	}	
#endif
...
#if 0	
        while (((cp->mii_if.mdio_read(dev , cp->mii_if.phy_id, 1)&0xffdb) != init_phy) 
		&& (i<10)) {
	...
	cp_reset_hw(cp);
        ...
        if((platform_info.board_id == realtek_neptune_qa_board)
           &&( (*(volatile unsigned int*)(0xb801a204)&0xffff0000) == 0xa00000))
                ...
                cp->mii_if.mdio_write(dev , cp->mii_if.phy_id, 0, 0x1200);
...
//	status = 0;
	...
	__cp_set_rx_mode(dev);
#ifdef CONFIG_JUPITER_ETH
	if (CHIP_REG == Jupiter_id) {
		if (!platform_info.ethtype){
		        printk("jupiter 10/100 \n");	
	                cpw32(Ethrnt_Ctrl, 0x43); //10/100
#if 1
			/* set PHY flow control */
			cp->mii_if.mdio_write(dev , cp->mii_if.phy_id, 4, 0x0de1);

			/* set power-saving mode */
			cp->mii_if.mdio_write(dev , cp->mii_if.phy_id, 0x18, 0x8310);
			/* set PHY PCS force mode to improve SNR*/
			cp->mii_if.mdio_write(dev , cp->mii_if.phy_id, 0x16, 0x5b8d);
			...
			printk("jupiter 1g \n");
                	cpw32(Ethrnt_Ctrl, 0x4b);   //1G
#if 1
			/* set PHY flow control */
			cp->mii_if.mdio_write(dev , cp->mii_if.phy_id, 4, 0x0fe1);
#endif

			if (cp->mii_if.mdio_read(dev , cp->mii_if.phy_id, 3) == 			    0xc912) {
				/* set External PHY Rx delay */
				cp->mii_if.mdio_write(dev , cp->mii_if.phy_id, 0x1f, 0x2);
	 			cp->mii_if.mdio_write(dev , cp->mii_if.phy_id, 0x18, 0x1a0);
				cp->mii_if.mdio_write(dev , cp->mii_if.phy_id, 0x1f, 0x0);

			}
		}	
		cpw32(Ethrnt_Ctrl1, 0x52080);
		cpw32(Ethrnt_Phase_Sel, 0x33a);
		//*(volatile unsigned int*)0xb801623c=0x4843;
#if 0
		/* auto negotiation */
		cp->mii_if.mdio_write(dev , cp->mii_if.phy_id, 0, 0x1200);
#endif
...
static int cp_init_one (void)
{
	...
	else if (CHIP_REG == Jupiter_id) {
	        rx_offset = 0;
	        printk("this  Jupiter eth RX_OFFSET = 0x%x\n", RX_OFFSET);
	}
	else {
	        rx_offset = 2;
	...
	dev->poll = cp_rx_poll;
	dev->weight = 32;//8;	/* arbitrary? from NAPI_HOWTO.txt. */
#ifdef BROKEN
