...
// static DECLARE_MUTEX (ptp_session_mutex);
...
#define CHECK_PTP_RC(result)	{uint16_t r=(result); if (r!=PTP_RC_OK) {ret = r ;goto err;} }

#define PTP_CNT_INIT(cnt) {memset(&cnt,0,sizeof(cnt));}

uint16_t ptp_exit_recv_memory_handler(PTPDataHandler *, unsigned char **, unsigned long*);
uint16_t ptp_init_recv_memory_handler(PTPDataHandler*);
uint16_t ptp_init_send_memory_handler(PTPDataHandler *, unsigned char *, unsigned long len);
uint16_t ptp_exit_send_memory_handler(PTPDataHandler *handler);
 ...
 **/
static uint16_t ptp_transaction_new(PTPParams *params, PTPContainer *ptp, uint16_t flags, unsigned int sendlen, PTPDataHandler *handler)
{
    int tries;
    uint16_t cmd;
    uint16_t ret = 0;
    ...
    if (params->obj_session.state == PtpObjectSessionData) 
    {        
        ret = ptp_getobject_session(params,0,NULL,0,true);
    ...
    switch (flags & PTP_DP_DATA_MASK)
    {
        case PTP_DP_SENDDATA:
            {                                    
                ...
                {
                    ret = params->cancelreq_func(params, params->transaction_id - 1);
                    if (ret == PTP_RC_OK)
    ...
    {       
        ...
        {
            tries++;
            ptp_debug(params, "PTP: Sequence number mismatch %d vs expected %d, suspecting old reply.", ptp->Transaction_ID, params->transaction_id - 1);
            continue;
        }
        if (ptp->Transaction_ID != params->transaction_id - 1)
        {
            ...
            if ((cmd == PTP_OC_OpenSession) && tries)
                continue;
            ptp_error(params, "PTP: Sequence number mismatch %d vs expected %d.", ptp->Transaction_ID, params->transaction_id - 1);
...
typedef enum _PTPMemHandlerPrivateMode
{
  PtpMemHandlerAllocate   = 0,
  PtpMemHandlerMapping    = 1, 
} PTPMemHandlerPrivateMode;

typedef struct
{
    PTPMemHandlerPrivateMode mode;
    unsigned char *data;
    unsigned long size, curoff;
} PTPMemHandlerPrivate;

static uint16_t memory_getfunc(PTPParams *params, void *private, unsigned long wantlen, unsigned char *data, unsigned long*gotlen)
{
    PTPMemHandlerPrivate *priv = (PTPMemHandlerPrivate*)private;
    unsigned long tocopy = wantlen;

    // printk("PTP (memory_getfunc) : priv->curoff = 0x%x , tocopy = %d , priv->size = %d , data = 0x%x , priv->data = 0x%x\n" , priv->curoff , tocopy , priv->size,data,priv->data);
   
    if (priv->curoff + tocopy > priv->size)
        tocopy = priv->size - priv->curoff;
        
    // printk("PTP (memory_getfunc) 2 : priv->curoff = 0x%x , tocopy = %d , priv->size = %d , data = 0x%x , priv->data = 0x%x\n" , priv->curoff , tocopy , priv->size,data,priv->data);
...
static uint16_t memory_putfunc(PTPParams *params, void *private, unsigned long sendlen, unsigned char *data, unsigned long *putlen)
{
    PTPMemHandlerPrivate *priv = (PTPMemHandlerPrivate*)private;

    // printk("PTP (memory_putfunc) : priv->curoff = 0x%x , sendlen = %d , priv->size = 0x%x , priv->data = 0x%x, data = 0x%x\n" , priv->curoff , sendlen , priv->size, priv->data, data);

    if ((priv->curoff + sendlen > priv->size) && (priv->mode == PtpMemHandlerAllocate))
    {        
        priv->data = LIBMTP_realloc(priv->data, priv->curoff + sendlen, priv->size);
        if (!priv->data)
    ...
    {        
        printk("%s [Error] : Buffer size too samll (priv->curoff = %d , sendlen = %d , priv->size = %d)\n",__func__,(u32) priv->curoff,(u32) sendlen,(u32) priv->size);  
        return  PTP_RC_GeneralError; 
    }    
    
    // printk("PTP (memory_putfunc) 2 : priv->curoff = 0x%x , sendlen = %d , priv->size = 0x%x , priv->data = 0x%x, data = 0x%x\n" , priv->curoff , sendlen , priv->size, priv->data, data);
...
static void *memory_get_data_ptr_func(PTPParams *params, void *private)
{
    PTPMemHandlerPrivate *priv = (PTPMemHandlerPrivate*)private;    
    return (void *) (priv->data + priv->curoff);
}

/*
static uint16_t memory_get_data_size_func(PTPParams *params, void *private , unsigned long *size)
{
    PTPMemHandlerPrivate *priv = (PTPMemHandlerPrivate*)private; 
    
    if (priv->data == NULL)
    {
        printk("%s [Error] : priv->data is NULL\n" , __func__);
        return PTP_RC_GeneralError;   
    }  
...
static uint16_t memory_set_data_ptr_func(PTPParams *params, void *private,void *data,unsigned long size)
{
    PTPMemHandlerPrivate *priv = (PTPMemHandlerPrivate*)private;
    
    if (priv->data != NULL)
    {
        printk("%s [Error] : priv->data is not NULL\n" , __func__);
        return PTP_RC_GeneralError;   
    }    
    
    priv->mode = PtpMemHandlerMapping;
    priv->data = data;
    priv->curoff = 0;
    priv->size = size;
    return PTP_RC_OK;
...
static uint16_t memory_set_data_offset_func(PTPParams *params, void *private,unsigned long offset)
{
    PTPMemHandlerPrivate *priv = (PTPMemHandlerPrivate*)private;
...
/*
static uint16_t memory_get_data_offset_func(PTPParams *params, void *private,unsigned long *offset)
{
    PTPMemHandlerPrivate *priv = (PTPMemHandlerPrivate*)private;
...
uint16_t ptp_init_recv_memory_handler(PTPDataHandler *handler)
{
    PTPMemHandlerPrivate *priv;
    priv = kmalloc(sizeof(PTPMemHandlerPrivate), GFP_KERNEL);
    if (!priv)
    {   
    ...
    handler->IsMemMapping = false;
    handler->getfunc = memory_getfunc;
    handler->putfunc = memory_putfunc;
    
    handler->GetDataPtrFunc    = memory_get_data_ptr_func;
    handler->SetDataPtrFunc    = memory_set_data_ptr_func;
    handler->SetDataOffsetFunc = memory_set_data_offset_func;  
    /*
    handler->GetDataOffsetFunc = memory_get_data_offset_func;
    handler->GetDataSizeFunc = memory_get_data_size_func;  
    */
    
    priv->mode = PtpMemHandlerAllocate;
    priv->data = NULL;
    priv->size = 0;
    priv->curoff = 0;   
    return PTP_RC_OK;
 ...
 */
uint16_t ptp_init_send_memory_handler(PTPDataHandler *handler, unsigned char *data, unsigned long len)
{
    PTPMemHandlerPrivate *priv;
    
    // printk("ptp_init_send_memory_handler : data= 0x%x , len = %d\n",data,len);      
...
uint16_t ptp_exit_recv_memory_handler(PTPDataHandler *handler, unsigned char **data, unsigned long *size)
{
    PTPMemHandlerPrivate *priv = (PTPMemHandlerPrivate*)handler->priv;        
    *data = priv->data;
    *size = priv->size;   
    
    // printk("ptp_exit_recv_memory_handler : data= 0x%x *data = 0x%x, *size = %d\n",data,*data,*size);     
...
static uint16_t ptp_transaction(PTPParams *params, PTPContainer *ptp, uint16_t flags, unsigned int sendlen, unsigned char**data, unsigned int *recvlen)
{
...
uint16_t ptp_getdeviceinfo(PTPParams *params, PTPDeviceInfo *deviceinfo)
{
    ...
    if (ret == PTP_RC_OK)
        ptp_unpack_DI(params, di, deviceinfo, len);
...
uint16_t ptp_canon_eos_getdeviceinfo(PTPParams *params, PTPCanonEOSDeviceInfo*di)
{
    ...
    ptp_exit_recv_memory_handler(&handler, &data, &len);
    if (ret == PTP_RC_OK)
        ptp_unpack_EOS_DI(params, data, di, len);
    kfree(data);
    return ret;
}
 ...
 * Return values: Some PTP_RC_* code.
 **/
uint16_t ptp_generic_no_data(PTPParams *params, uint16_t code, unsigned int n_param, ...)
{
    PTPContainer ptp;
    va_list args;
    int i;
...
void ptp_free_params(PTPParams *params)
{
    ...
    if (params->cameraname)
        kfree(params->cameraname);
    if (params->wifi_profiles)
        kfree(params->wifi_profiles);
        
    // printk("%s : params->nrofobjects = %d\n",__func__,params->nrofobjects);
        
    for (i = 0; i < params->nrofobjects; i++)
    {
        // printk("%s : params->objects[%d] = 0x%x\n",__func__,i,&params->objects[i]);
...
uint16_t ptp_getstorageids(PTPParams *params, PTPStorageIDs *storageids)
{
    ...
    if (ret == PTP_RC_OK)
        ptp_unpack_SIDs(params, sids, storageids, len);
    kfree(sids);
    return ret;
}
...
uint16_t ptp_getstorageinfo(PTPParams *params, uint32_t storageid, PTPStorageInfo *storageinfo)
{
    ...
    if (ret == PTP_RC_OK)
        ptp_unpack_SI(params, si, storageinfo, len);
    kfree(si);
    return ret;
}
 ...
 * Return values: Some PTP_RC_* code.
 **/
uint16_t ptp_getobjecthandles(PTPParams *params, uint32_t storage, uint32_t objectformatcode, uint32_t associationOH, PTPObjectHandles *objecthandles)
{
    uint16_t ret;
    PTPContainer ptp;
    ...
    {
        if ((storage == 0xffffffff) && (objectformatcode == 0) && (associationOH == 0))
        {
 ...
 * Return values: Some PTP_RC_* code.
 **/
uint16_t ptp_getnumobjects(PTPParams *params, uint32_t storage, uint32_t objectformatcode, uint32_t associationOH, uint32_t*numobs)
{
    uint16_t ret;
    PTPContainer ptp;
    int len;
 ...
 * Return values: Some PTP_RC_* code.
 **/
uint16_t ptp_getobject(PTPParams *params, uint32_t handle, unsigned char**object)
{
    PTPContainer ptp;
    unsigned int len;
 ...
 * Return values: Some PTP_RC_* code.
 **/
uint16_t ptp_getobject_to_handler(PTPParams *params, uint32_t handle, PTPDataHandler *handler)
{
    PTPContainer ptp;
...
static inline uint16_t ptp_getobject_session_end(PTPParams *params)
{
    int tries;
    uint16_t ret;
    
    PTPContainer *ptp = &params->obj_session.ptp;
    ...
    while (tries--)
    {
        ...
        // printk("%s : ret = 0x%x , ptp->Transaction_ID = %d , params->transaction_id = %d\n",__func__,ret,ptp->Transaction_ID,params->transaction_id);
...
/**
 * ptp_getobject_session:
 * params:	PTPParams*
 *		handle			- Object handle
 *		buf  			- pointer to data area
 *      size            - data size
 *      reset           - reset seesion state machine
 *
 * Get object 'handle' from device and store the data in newly
 * allocated 'object'.
 *
 * Return values: Some PTP_RC_* code.
 **/
uint16_t ptp_getobject_session(PTPParams *params, uint32_t handle, char *buf, uint32_t size, bool reset)
{
    uint16_t ret = 0;
    PTPContainer *ptp = &params->obj_session.ptp;
    PTP_USB *ptp_usb = (PTP_USB *) params->data;
    
    // printk("reset = %s , params->obj_session.state = %d\n",reset ? "True" : "False",params->obj_session.state);
    
    // down(&ptp_session_mutex);
    down(&params->session_mutex_lock);
                   
    if (reset == true)
    {
        if (params->obj_session.state == PtpObjectSessionClose) 
        {    
            memset(&params->obj_session,0,sizeof(PTPObjectSession));  
            // up(&ptp_session_mutex);          
            up(&params->session_mutex_lock);
            return PTP_RC_OK; 
        }  
...
_SessionStar :
    ...
    {            
        down(&params->mutex_lock);        
        params->obj_session.handle = handle;
        params->obj_session.cur_offset = 0;        
        params->obj_session.data = buf;
        params->obj_session.req_size = size;        
        params->obj_session.state = PtpObjectSessionOpen;
    }    
    else if ((params->obj_session.handle == handle) && (params->obj_session.state == PtpObjectSessionData))
    {
        params->obj_session.data = buf;
        params->obj_session.req_size = size;
        params->obj_session.state = PtpObjectSessionData;                
    }
    else
    {                    
        goto err;   
    ...
    switch (params->obj_session.state)
    {
        case PtpObjectSessionOpen :  
            // printk("PtpObjectSessionOpen\n"); 
            
            PTP_CNT_INIT(params->obj_session.ptp);
            params->obj_session.ptp.Code = PTP_OC_GetObject;
            params->obj_session.ptp.Param1 = handle;
            params->obj_session.ptp.Nparam = 1;
            
            params->obj_session.ptp.Transaction_ID = params->transaction_id++;
            params->obj_session.ptp.SessionID = params->session_id;
            
            // Creative ZEN VISION:M Delay 10 ms?
            // mdelay(20);         
            CHECK_PTP_RC(params->sendreq_func(params, ptp)); 
            params->obj_session.state = PtpObjectSessionData; 
            // printk("PtpObjectSessionOpen End\n");             
            goto _SessionStar;
        break;
        
        case PtpObjectSessionData :
        {    
            ...
            // printk("PtpObjectSessionData : size = %d\n",size);
                                   
            ptp_init_recv_memory_handler(&handler); 
            handler.mode = PtpDataHandlerModeSession;   
            
            if (buf)
            {    
                handler.IsMemMapping = true;
                ret = handler.SetDataPtrFunc(params,handler.priv,buf,size);
            ...
            ptp_exit_recv_memory_handler(&handler, &data, &rlen);
            
            // printk("ret = 0x%x , rlen = %d\n",ret , rlen);
            ...
            if ((handler.IsMemMapping == false) && data)
            {
                kfree(data);    
            }    
            ...
            */   
            ...
            // printk("\n\nparams->obj_session.cur_offset = 0x%x , params->obj_session.obj_size = 0x%x\n",params->obj_session.cur_offset , params->obj_session.obj_size);
            
            if (params->obj_session.cur_offset >= params->obj_session.obj_size)
            {
                ret = ptp_getobject_session_end(params); 
                ...
                // memset(&params->obj_session,0,sizeof(PTPObjectSession)); 
                params->obj_session.state = PtpObjectSessionClose;
            ...
            // printk("PtpObjectSessionData : End\n");
            // up(&ptp_session_mutex);
            up(&params->session_mutex_lock);                           
                
            return PTP_NRC_GETOBJECT;                  
        }        
        break;
...
reset :  
err :      
    if ((!(ptp_usb->flags & ABORTING_OR_DISCONNECTING)) && (params->obj_session.state == PtpObjectSessionData))
    {    
        uint16_t cancel_ret;
        cancel_ret = params->cancelreq_func(params, params->transaction_id - 1);  
        if (cancel_ret == PTP_RC_OK)
        {
            if (reset == true)
                ret = PTP_RC_OK;
            else         
        ...
        // printk("%s : cancel request\n",__func__);                
    }        
                
    memset(&params->obj_session,0,sizeof(PTPObjectSession));  
            
    down_trylock(&params->mutex_lock);  
    up(&params->mutex_lock); 
 ...
 * Return values: Some PTP_RC_* code.
 **/
uint16_t ptp_getpartialobject(PTPParams *params, uint32_t handle, uint32_t offset, uint32_t maxbytes, unsigned char **object, uint32_t *len)
{
    PTPContainer ptp;
 ...
 * Return values: Some PTP_RC_* code.
 **/
uint16_t ptp_getpartialobject_new(PTPParams *params, uint32_t handle, uint32_t offset, uint32_t maxbytes, unsigned char *object, uint32_t *len)
{
    PTPDataHandler handler;
    PTPContainer ptp;   
    uint16_t ret;  
    ...
    ptp_init_recv_memory_handler(&handler);    
    handler.IsMemMapping = true;
    ret = handler.SetDataPtrFunc(params,handler.priv,object,maxbytes);
    ...
    ret = ptp_transaction_new(params, &ptp, PTP_DP_GETDATA , 0, &handler);          
    ptp_exit_recv_memory_handler(&handler, &data, (unsigned long *) len);
 ...
 * Return values: Some PTP_RC_* code.
 **/
uint16_t ptp_getthumb(PTPParams *params, uint32_t handle, unsigned char**object)
{
    PTPContainer ptp;
    unsigned int len;
 ...
 **/
uint16_t ptp_sendobjectinfo(PTPParams *params, uint32_t *store, uint32_t*parenthandle, uint32_t *handle, PTPObjectInfo*objectinfo)
{
    uint16_t ret;
    PTPContainer ptp;
 ...
 *
 */
uint16_t ptp_sendobject(PTPParams *params, unsigned char *object, uint32_t size)
{
    PTPContainer ptp;
 ...
 * Return values: Some PTP_RC_* code.
 **/
uint16_t ptp_sendobject_from_handler(PTPParams *params, PTPDataHandler *handler, uint32_t size)
{
    PTPContainer ptp;
...
uint16_t ptp_getdevicepropvalue(PTPParams *params, uint16_t propcode, PTPPropertyValue *value, uint16_t datatype)
{
    PTPContainer ptp;
    uint16_t ret;
    unsigned int len;
    ...
    if (ret == PTP_RC_OK)
        ptp_unpack_DPV(params, dpv, &offset, len, value, datatype);
    kfree(dpv);
    return ret;
}

uint16_t ptp_setdevicepropvalue(PTPParams *params, uint16_t propcode, PTPPropertyValue *value, uint16_t datatype)
{
    PTPContainer ptp;
    uint16_t ret;
    uint32_t size;
 ...
 *					  for the incoming object
 **/
uint16_t ptp_ek_sendfileobjectinfo(PTPParams *params, uint32_t *store, uint32_t*parenthandle, uint32_t *handle, PTPObjectInfo *objectinfo)
{
    uint16_t ret;
    PTPContainer ptp;
    unsigned char *oidata = NULL;
 ...
 *
 */
uint16_t ptp_ek_getserial(PTPParams *params, unsigned char **data, unsigned int*size)
{
    PTPContainer ptp;
 ...
 *
 */
uint16_t ptp_ek_setserial(PTPParams *params, unsigned char *data, unsigned int size)
{
    PTPContainer ptp;
...
uint16_t ptp_ek_9007(PTPParams *params, unsigned char **data, unsigned int*size)
{
    PTPContainer ptp;
...
/* unclear what it does yet */
uint16_t ptp_ek_9009(PTPParams *params, uint32_t *p1, uint32_t *p2)
{
    PTPContainer ptp;
    uint16_t ret;
...
uint16_t ptp_ek_900c(PTPParams *params, unsigned char **data, unsigned int*size)
{
    PTPContainer ptp;
 ...
 *
 */
uint16_t ptp_ek_sendfileobject(PTPParams *params, unsigned char *object, uint32_t size)
{
    PTPContainer ptp;
 ...
 *
 */
uint16_t ptp_ek_sendfileobject_from_handler(PTPParams *params, PTPDataHandler*handler, uint32_t size)
{
    PTPContainer ptp;
 ...
 *
 **/
uint16_t ptp_canon_getpartialobjectinfo(PTPParams *params, uint32_t handle, uint32_t p2, uint32_t *size, uint32_t*rp2)
{
    uint16_t ret;
    PTPContainer ptp;
 ...
 *
 **/
uint16_t ptp_canon_get_directory(PTPParams *params, PTPObjectHandles *handles, PTPObjectInfo **oinfos,  /* size(handles->n) */
 ...
 *
 **/
uint16_t ptp_canon_getpairinginfo(PTPParams *params, uint32_t nr, unsigned char**data, unsigned int *size)
{
    PTPContainer ptp;
    uint16_t ret;
 ...
 *
 **/
uint16_t ptp_canon_gettreesize(PTPParams *params, PTPCanon_directtransfer_entry**entries, unsigned int *cnt)
{
    PTPContainer ptp;
    uint16_t ret;
    ...
    if (ret != PTP_RC_OK)
        return ret;
    *cnt = dtoh32a(out);
    *entries = kmalloc(sizeof(PTPCanon_directtransfer_entry)*(*cnt), GFP_KERNEL);
    if (! *entries)
    {
        kfree(out);
        return PTP_RC_GeneralError;
 ...
 *						  or 0 otherwise
 **/
uint16_t ptp_canon_checkevent(PTPParams *params, PTPContainer *event, int*isevent)
{
    uint16_t ret;
    PTPContainer ptp;
...
uint16_t ptp_check_event(PTPParams *params)
{
    ...
    if ((params->deviceinfo.VendorExtensionID == PTP_VENDOR_NIKON) && ptp_operation_issupported(params, PTP_OC_NIKON_CheckEvent))
    {
        int evtcnt;
        ...
        if (evtcnt)
        {
            if (params->nrofevents)
                params->events = LIBMTP_realloc(params->events, sizeof(PTPContainer)*(evtcnt + params->nrofevents), sizeof(PTPContainer) *evtcnt);
            else
                params->events = kmalloc(sizeof(PTPContainer) *evtcnt, GFP_KERNEL);
    ...
    if ((params->deviceinfo.VendorExtensionID == PTP_VENDOR_CANON) && ptp_operation_issupported(params, PTP_OC_CANON_CheckEvent))
    {
        int isevent;
    ...
    store_event: if (ret == PTP_RC_OK)
    {
        ...
        if (params->nrofevents)
            params->events = LIBMTP_realloc(params->events, sizeof(PTPContainer)*(params->nrofevents + 1), sizeof(PTPContainer) *params->nrofevents);
        else
            params->events = kmalloc(sizeof(PTPContainer) *1, GFP_KERNEL);
...
int ptp_get_one_event(PTPParams *params, PTPContainer *event)
{
    if (!params->nrofevents)
        return 0;
...
uint16_t ptp_check_eos_events(PTPParams *params)
{
    ...
    while (1)
    {
        /* call it repeatedly until the camera does not report any */
        ...
        {
            nentries = LIBMTP_realloc(params->backlogentries, sizeof(entries[0])*(params->nrofbacklogentries + nrofentries), sizeof(entries[0]) *params->nrofbacklogentries);
            if (!nentries)
                return PTP_RC_GeneralError;
...
uint16_t ptp_canon_eos_getdevicepropdesc(PTPParams *params, uint16_t propcode, PTPDevicePropDesc *dpd)
{
    int i;
    ...
    if (dpd->FormFlag == PTP_DPFF_Enumeration)
    {
        ...
        dpd->FORM.Enum.SupportedValue = kmalloc(sizeof(PTPPropertyValue) *dpd->FORM.Enum.NumberOfValues, GFP_KERNEL);
...
uint16_t ptp_canon_eos_getstorageinfo(PTPParams *params, uint32_t p1, unsigned char **data, unsigned int *size)
{
    PTPContainer ptp;
    uint16_t ret;
 ...
 *
 */
uint16_t ptp_canon_eos_getpartialobject(PTPParams *params, uint32_t oid, uint32_t offset, uint32_t xsize, unsigned char **data)
{
    PTPContainer ptp;
    unsigned int size = 0;
...
uint16_t ptp_canon_eos_getpartialobject_new(PTPParams *params, uint32_t oid, uint32_t offset, uint32_t xsize, unsigned char **object)
{
    PTPDataHandler handler;
    PTPContainer ptp;   
    uint16_t ret;  
    ...
    ptp_init_recv_memory_handler(&handler);    
    handler.IsMemMapping = true;
    ret = handler.SetDataPtrFunc(params,handler.priv,object,xsize);
...
uint16_t ptp_canon_eos_setdevicepropvalue(PTPParams *params, uint16_t propcode, PTPPropertyValue *value, uint16_t datatype)
{
    PTPContainer ptp;
    uint16_t ret;
    int i;
    ...
    switch (propcode)
    {
        ...
        default:
            if (datatype != PTP_DTC_STR)
            {
                ...
                data = kmalloc(sizeof(uint32_t) *3, GFP_KERNEL);
                memset(data,0,sizeof(uint32_t) *3);
            ...
            case PTP_DTC_STR:
                ...
                kfree(params->canon_props[i].dpd.CurrentValue.str);
 ...
 *
 **/
uint16_t ptp_canon_getpartialobject(PTPParams *params, uint32_t handle, uint32_t offset, uint32_t size, uint32_t pos, unsigned char **block, uint32_t*readnum)
{
    uint16_t ret;
    PTPContainer ptp;
    unsigned char *data = NULL;
...
uint16_t ptp_canon_getpartialobject_new(PTPParams *params, uint32_t handle, uint32_t offset, uint32_t size, uint32_t pos, unsigned char **object, uint32_t*readnum)
{
    PTPDataHandler handler;
    PTPContainer ptp;   
    uint16_t ret;  
    ...
    ptp_init_recv_memory_handler(&handler);    
    handler.IsMemMapping = true;
    ret = handler.SetDataPtrFunc(params,handler.priv,object,size);
 ...
 *
 **/
uint16_t ptp_canon_getviewfinderimage(PTPParams *params, unsigned char **image, uint32_t *size)
{
    uint16_t ret;
    PTPContainer ptp;
    unsigned int len;
 ...
 *
 **/
uint16_t ptp_canon_getobjectinfo(PTPParams *params, uint32_t store, uint32_t p2, uint32_t parent, uint32_t handle, PTPCANONFolderEntry **entries, uint32_t*entnum)
{
    uint16_t ret;
    PTPContainer ptp;
    unsigned char *data = NULL;
    ...
    if (ret == PTP_RC_OK)
    {
        ...
        *entries = kmalloc(*entnum * sizeof(PTPCANONFolderEntry), GFP_KERNEL);
        memset(*entries,0,*entnum * sizeof(PTPCANONFolderEntry));
...
uint16_t ptp_canon_get_objecthandle_by_name(PTPParams *params, char *name, uint32_t *objectid)
{
    ...
    ptp.Nparam = 0;
    len = 0;
    data = kmalloc(2 *(strlen(name) + 1) + 2, GFP_KERNEL);
 ...
 *
 **/
uint16_t ptp_canon_get_customize_data(PTPParams *params, uint32_t themenr, unsigned char **data, unsigned int *size)
{
    PTPContainer ptp;
...
uint16_t ptp_nikon_curve_download(PTPParams *params, unsigned char **data, unsigned int *size)
{
    PTPContainer ptp;
    *data = NULL;
    *size = 0;
...
uint16_t ptp_nikon_getfileinfoinblock(PTPParams *params, uint32_t p1, uint32_t p2, uint32_t p3, unsigned char **data, unsigned int *size)
{
    PTPContainer ptp;
    *data = NULL;
    *size = 0;
 ...
 *
 **/
uint16_t ptp_nikon_get_liveview_image(PTPParams *params, unsigned char **data, unsigned int *size)
{
    PTPContainer ptp;
 ...
 *
 **/
uint16_t ptp_nikon_get_preview_image(PTPParams *params, unsigned char **xdata, unsigned int *xsize, uint32_t *handle)
{
    PTPContainer ptp;
    uint16_t ret;
 ...
 *
 **/
uint16_t ptp_nikon_check_event(PTPParams *params, PTPContainer **event, int*evtcnt)
{
    PTPContainer ptp;
    uint16_t ret;
    unsigned char *data = NULL;
...
uint16_t ptp_nikon_getwifiprofilelist(PTPParams *params)
{
   ...
   // down(&params->mutex_lock);
    ...
    /* FIXME: Add more precise error code */
    ...
    params->wifi_profiles = kmalloc(params->wifi_profiles_number *sizeof(PTPNIKONWifiProfile), GFP_KERNEL);
...
uint16_t ptp_nikon_writewifiprofile(PTPParams *params, PTPNIKONWifiProfile *profile)
{
    ...
    /* IP parameters */
    *((unsigned int*) &buffer[0x3A]) = profile->ip_address; /* Do not reverse
    bytes */
    buffer[0x3E] = profile->subnet_mask;
    *((unsigned int*) &buffer[0x3F]) = profile->gateway_address; /* Do not
    reverse bytes */
 ...
 *
 **/
uint16_t ptp_mtp_getobjectpropdesc(PTPParams *params, uint16_t opc, uint16_t ofc, PTPObjectPropDesc *opd)
{
    PTPContainer ptp;
    uint16_t ret;
    unsigned char *data = NULL;
 ...
 *
 **/
uint16_t ptp_mtp_getobjectpropvalue(PTPParams *params, uint32_t oid, uint16_t opc, PTPPropertyValue *value, uint16_t datatype)
{
    PTPContainer ptp;
    uint16_t ret;
    unsigned char *data = NULL;
 ...
 *
 **/
uint16_t ptp_mtp_setobjectpropvalue(PTPParams *params, uint32_t oid, uint16_t opc, PTPPropertyValue *value, uint16_t datatype)
{
    PTPContainer ptp;
    uint16_t ret;
    unsigned char *data = NULL;
...
uint16_t ptp_mtp_getobjectproplist(PTPParams *params, uint32_t handle, MTPProperties **props, int *nrofprops)
{
    ...
    if (ret == PTP_RC_OK)
        *nrofprops = ptp_unpack_OPL(params, opldata, props, oplsize);
    if (opldata != NULL)
        kfree(opldata);
    return ret;
}

uint16_t ptp_mtp_sendobjectproplist(PTPParams *params, uint32_t *store, uint32_t *parenthandle, uint32_t *handle, uint16_t objecttype, uint64_t objectsize, MTPProperties *props, int nrofprops)
{
    uint16_t ret;
    PTPContainer ptp;
    unsigned char *opldata = NULL;
    ...
    switch (dpd->FormFlag)
    {
        ...
        case PTP_DPFF_Enumeration:
            if (dpd->FORM.Enum.SupportedValue)
            {
                ...
                kfree(dpd->FORM.Enum.SupportedValue);
            }
    }
}
    ...
    switch (opd->FormFlag)
    {
        ...
        case PTP_OPFF_Enumeration:
            if (opd->FORM.Enum.SupportedValue)
            {
                ...
                kfree(opd->FORM.Enum.SupportedValue);
            }
            break;
        ...
        case PTP_OPFF_LongString:
            ...
            break;
        default:
            printk(KERN_ERR "Unknown OPFF type %d\n", opd->FormFlag);
            break;
    }
}
...
{
    // printk("%s : oi = 0x%x , oi->Filename = 0x%x , __func__, oi->Keywords = 0x%x\n",oi ,oi->Filename,oi->Keywords);
    if (!oi)
        return ;
...
const char *ptp_get_property_description(PTPParams *params, uint16_t dpc)
{
    ...
    {
        {
            PTP_DPC_FUJI_ColorTemperature, N_("Color Temperature")
        }
        ,  /* 0xD017 */
        {
            PTP_DPC_FUJI_Quality, N_("Quality")
        }
        ,  /* 0xD018 */
        {
            PTP_DPC_FUJI_Quality, N_("Release Mode")
        }
        ,  /* 0xD201 */
        {
            PTP_DPC_FUJI_Quality, N_("Focus Areas")
        }
        ,  /* 0xD206 */
        {
            PTP_DPC_FUJI_Quality, N_("AE Lock")
        }
        ,  /* 0xD213 */
        {
            PTP_DPC_FUJI_Quality, N_("Aperture")
        }
        ,  /* 0xD218 */
        {
            PTP_DPC_FUJI_Quality, N_("Shutter Speed")
        }
        ,  /* 0xD219 */
        {
            0, NULL
    ...
    if (params->deviceinfo.VendorExtensionID == PTP_VENDOR_MICROSOFT || params->deviceinfo.VendorExtensionID == PTP_VENDOR_MTP)
...
#if 0
static int64_t _value_to_num(PTPPropertyValue *data, uint16_t dt)
{
    if (dt == PTP_DTC_STR)
    {
        if (!data->str)
        ...
        return simple_strtoll(data->str, NULL, 10);
    }
...
#if 0
    int ptp_render_property_value(PTPParams *params, uint16_t dpc, PTPDevicePropDesc *dpd, int length, char *out)
    {
        int i;
        int64_t kval;
        ...
        } ptp_value_trans[] = 
        {
            ...
            ,  /* 5010 */
            {
                PTP_DPC_CaptureDelay, 0, 0.001, 0.0, "%.1fs"
            }
            ,  /* 5012 */
            {
            ...
            ,  /* 5016 */
            {
                PTP_DPC_BurstInterval, 0, 0.001, 0.0, "%.1fs"
            }
            ,  /* 5019 */
        ...
        } ptp_value_list[] = 
        {
            ...
            /* Canon stuff */
            PTP_VENDOR_VAL_BOOL(PTP_DPC_CANON_AssistLight, PTP_VENDOR_CANON), PTP_VENDOR_VAL_RBOOL(PTP_DPC_CANON_RotationScene, PTP_VENDOR_CANON), PTP_VENDOR_VAL_RBOOL(PTP_DPC_CANON_BeepMode, PTP_VENDOR_CANON), PTP_VENDOR_VAL_BOOL(PTP_DPC_CANON_Beep, PTP_VENDOR_CANON), 
        ...
        {
            if ((ptp_value_trans[i].dpc == dpc) && (((ptp_value_trans[i].dpc &0xf000) == 0x5000) || (ptp_value_trans[i].vendor == params->deviceinfo.VendorExtensionID)))
            {
                double value = _value_to_num(&(dpd->CurrentValue), dpd->DataType);

                return snprintf(out, length, _(ptp_value_trans[i].format), value *ptp_value_trans[i].coef + ptp_value_trans[i].bias);
            }
        }
        ...
        {
            if ((ptp_value_list[i].dpc == dpc) && (((ptp_value_list[i].dpc &0xf000) == 0x5000) || (ptp_value_list[i].vendor == params->deviceinfo.VendorExtensionID)) && (ptp_value_list[i].key == kval))
            {
                ...
                case PTP_DPC_MTP_DeviceCertificate:
                    {
                        ...
                        if (dpd->CurrentValue.a.count && (dpd->CurrentValue.a.count < length))
                        {
...
MTPProperties *ptp_get_new_object_prop_entry(MTPProperties **props, int*nrofprops)
{
    ...
    if (*props == NULL)
    {
        newprops = kmalloc(sizeof(MTPProperties)*(*nrofprops + 1), GFP_KERNEL);
    }
    else
    {
        newprops = LIBMTP_realloc(*props, sizeof(MTPProperties)*(*nrofprops + 1), sizeof(MTPProperties)*(*nrofprops));
    }
    if (newprops == NULL)
        return NULL;
...
void ptp_destroy_object_prop(MTPProperties *prop)
{
    ...
    if (prop->datatype == PTP_DTC_STR && prop->propval.str != NULL)
        kfree(prop->propval.str);
    else if ((prop->datatype == PTP_DTC_AINT8 || prop->datatype == PTP_DTC_AINT16 || prop->datatype == PTP_DTC_AINT32 || prop->datatype == PTP_DTC_AINT64 || prop->datatype == PTP_DTC_AINT128 || prop->datatype == PTP_DTC_AUINT8 || prop->datatype == PTP_DTC_AUINT16 || prop->datatype == PTP_DTC_AUINT32 || prop->datatype == PTP_DTC_AUINT64 || prop->datatype == PTP_DTC_AUINT128) && prop->propval.a.v != NULL)
        kfree(prop->propval.a.v);
}
...
void ptp_remove_object_from_cache(PTPParams *params, uint32_t handle)
{
    ...
    params->objects = LIBMTP_realloc(params->objects, sizeof(PTPObject) *params->nrofobjects, sizeof(PTPObject) *params->nrofobjects);
}
...
uint16_t ptp_object_find(PTPParams *params, uint32_t handle, PTPObject **retob)
{
    ...
    tmpob.oid = handle;
    *retob = (PTPObject*)bsearch(&tmpob, params->objects, params->nrofobjects, sizeof(tmpob), _cmp_ob);
    if (! *retob)
        return PTP_RC_GeneralError;
    return PTP_RC_OK;
}
...
uint16_t ptp_object_find_or_insert(PTPParams *params, uint32_t handle, PTPObject **retob)
{
    ...
    *retob = NULL;
    if (!params->nrofobjects)
    {
        ...
        params->objects = kmalloc(1 *sizeof(PTPObject), GFP_KERNEL);
        memset(params->objects,0,sizeof(PTPObject));
    ...
    newobs = LIBMTP_realloc(params->objects, sizeof(PTPObject)*(params->nrofobjects + 1), sizeof(PTPObject) *params->nrofobjects);
    if (!newobs)
        return PTP_RC_GeneralError;
...
uint16_t ptp_object_want(PTPParams *params, uint32_t handle, int want, PTPObject **retob)
{
    uint16_t ret;
    PTPObject *ob;
