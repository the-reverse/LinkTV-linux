...
static char *ptp_unpack_unincode_string(PTPParams *params, unsigned char* data, uint16_t offset, uint8_t *len)
{
    int i;
	uint8_t length;	
	uint16_t *string;
	...
	*len = srclen;
    ...
    string = (uint16_t *) kmalloc(srclen + 2,GFP_KERNEL);
    ...
    {
        memcpy(string,&data[offset+1],srclen);
    }
    else
    {        
	    ...
	    for (i=0;i<length;i++) 
	    {		
		    string[i] = dtoh16a(&data[offset+1+2*i]);
	    }
    }
...
static char *ptp_unpack_string(PTPParams *params, unsigned char* data, uint16_t offset, uint8_t *len)
{
	uint8_t length;
	...
	*dest = '\0';
	...
	return((char *) strdup((char *) &loclstr[0]));
}
...
static void ptp_pack_string(PTPParams *params, char *string, unsigned char* data, uint16_t offset, uint8_t *len)
{
	int packedlen;
...
static unsigned char *ptp_get_packed_stringcopy(PTPParams *params, char *string, uint32_t *packed_size)
{
	...
	retcopy = kmalloc(plen,GFP_KERNEL);
	if (!retcopy) {
		*packed_size = 0;
		return NULL;
	}
...
ptp_unpack_uint32_t_array(PTPParams *params, unsigned char* data, uint16_t offset, uint32_t **array)
{
	...
	*array = kmalloc (n*sizeof(uint32_t),GFP_KERNEL);
	while (n>i) {
...
ptp_pack_uint32_t_array(PTPParams *params, uint32_t *array, uint32_t arraylen, unsigned char **data )
{
	...
	*data = kmalloc ((arraylen+1)*sizeof(uint32_t),GFP_KERNEL);
...
ptp_unpack_uint16_t_array(PTPParams *params, unsigned char* data, uint16_t offset, uint16_t **array)
{
	...
	n=dtoh32a(&data[offset]);
	*array = kmalloc (n*sizeof(uint16_t),GFP_KERNEL);
	while (n>i) {
...
ptp_free_DI (PTPDeviceInfo *di) {
	if (di->SerialNumber) kfree (di->SerialNumber);
	if (di->DeviceVersion) kfree (di->DeviceVersion);
	if (di->Model) kfree (di->Model);
	if (di->Manufacturer) kfree (di->Manufacturer);
	if (di->ImageFormats) kfree (di->ImageFormats);
	if (di->CaptureFormats) kfree (di->CaptureFormats);
	if (di->VendorExtensionDesc) kfree (di->VendorExtensionDesc);
	if (di->OperationsSupported) kfree (di->OperationsSupported);
	if (di->EventsSupported) kfree (di->EventsSupported);
	if (di->DevicePropertiesSupported) kfree (di->DevicePropertiesSupported);
}
...
{
	kfree (di->EventsSupported);
	kfree (di->DevicePropertiesSupported);
	kfree (di->unk);
}
...
ptp_pack_OI (PTPParams *params, PTPObjectInfo *oi, unsigned char** oidataptr)
{
	...
	oidata = kmalloc(PTP_oi_MaxLen,GFP_KERNEL);
...
ptp_unpack_PTPTIME (const char *str) {
	...
	tmp[4] = 0;
	tm.tm_year=simple_strtol(tmp,NULL,10) - 1900;
	strncpy (tmp, ptpdate + 4, 2);
	tmp[2] = 0;
	tm.tm_mon = simple_strtol(tmp,NULL,10) - 1;
	strncpy (tmp, ptpdate + 6, 2);
	tmp[2] = 0;
	tm.tm_mday = simple_strtol(tmp,NULL,10);
	strncpy (tmp, ptpdate + 9, 2);
	tmp[2] = 0;
	tm.tm_hour = simple_strtol(tmp,NULL,10);
	strncpy (tmp, ptpdate + 11, 2);
	tmp[2] = 0;
	tm.tm_min = simple_strtol(tmp,NULL,10);
	strncpy (tmp, ptpdate + 13, 2);
	tmp[2] = 0;
	tm.tm_sec = simple_strtol(tmp,NULL,10);
	// return mktime (&tm);
...
ptp_unpack_OI (PTPParams *params, unsigned char* data, PTPObjectInfo *oi, unsigned int len)
{
	...
	oi->Filename= ptp_unpack_unincode_string(params, data, PTP_oi_filenamelen, &filenamelen);
    oi->Filename_len = filenamelen;
...
#define RARR(val,member,func)	{			\
	...
	*offset += sizeof(uint32_t);			\
							\
	val->a.count = n;				\
	val->a.v = kmalloc(sizeof(val->a.v[0])*n,GFP_KERNEL);	\
	if (!val->a.v) return 0;			\
	for (j=0;j<n;j++)				\
...
#define N	dpd->FORM.Enum.NumberOfValues
		...
		dpd->FORM.Enum.SupportedValue = kmalloc(N*sizeof(dpd->FORM.Enum.SupportedValue[0]),GFP_KERNEL);
...
#define N	opd->FORM.Enum.NumberOfValues
		N = dtoh16a(&data[offset]);
		offset+=sizeof(uint16_t);
		opd->FORM.Enum.SupportedValue = kmalloc(N*sizeof(opd->FORM.Enum.SupportedValue[0]),GFP_KERNEL);
...
static  uint32_t ptp_pack_OPL (PTPParams *params, MTPProperties *props, int nrofprops, unsigned char** opldataptr)
{
	...
	opldata = kmalloc(totalsize,GFP_KERNEL);
...
{ 
	...
	len -= sizeof(uint32_t);
	props = kmalloc(prop_count * sizeof(MTPProperties),GFP_KERNEL);
	if (!props) return 0;
...
ptp_unpack_CANON_changes (PTPParams *params, unsigned char* data, int datasize, PTPCanon_changes_entry **ce)
{
	...
	*ce = kmalloc (sizeof(PTPCanon_changes_entry)*(entries+1),GFP_KERNEL);
	if (!*ce) return 0;
	...
	while (curdata - data < datasize) {
		...
		case  PTP_EC_CANON_EOS_AvailListChanged: {	/* property desc */
			...
			if (dpd->FORM.Enum.SupportedValue) kfree (dpd->FORM.Enum.SupportedValue);
			dpd->FORM.Enum.SupportedValue = kmalloc (sizeof (PTPPropertyValue)*propxcnt,GFP_KERNEL);
			...
			if (size >= 0xc) {	/* property info */
				...
				if (j<params->nrofcanon_props) {
					if ((params->canon_props[j].size != size) ||
						...
						params->canon_props[j].data = LIBMTP_realloc(params->canon_props[j].data,size-PTP_ece_Prop_Val_Data,params->canon_props[j].size);
						memcpy (params->canon_props[j].data,data,size-PTP_ece_Prop_Val_Data);
					}
				} else {
					if (j)
						params->canon_props = LIBMTP_realloc(params->canon_props, sizeof(params->canon_props[0])*(j+1), sizeof(params->canon_props[0])* j);
					else
						params->canon_props = kmalloc(sizeof(params->canon_props[0]),GFP_KERNEL);
					params->canon_props[j].type = type;
					params->canon_props[j].proptype = proptype;
					params->canon_props[j].size = size;
					params->canon_props[j].data = kmalloc(size-PTP_ece_Prop_Val_Data,GFP_KERNEL);
					memcpy(params->canon_props[j].data, data, size-PTP_ece_Prop_Val_Data);
...
#else
					if (dpd->FactoryDefaultValue.str) kfree (dpd->FactoryDefaultValue.str);
					dpd->FactoryDefaultValue.str	= strdup( (char*)data );

					if (dpd->CurrentValue.str) kfree (dpd->CurrentValue.str);
...
#define XX(x)		case PTP_EC_CANON_EOS_##x: 								\
				...
				(*ce)[i].u.info = kmalloc(strlen("unhandled EOS event "#x" (size 123456789)"),GFP_KERNEL);	\
...
ptp_unpack_Nikon_EC (PTPParams *params, unsigned char* data, unsigned int len, PTPContainer **ec, int *cnt)
{
	...
	*ec = kmalloc(sizeof(PTPContainer)*(*cnt),GFP_KERNEL);
...
#define ISOBJECT(ptr) (dtoh32a((ptr)+ptp_canon_dir_storageid) == 0xffffffff)
	...
	handles->Handler = kmalloc(sizeof(handles->Handler[0]) * nrofobs,GFP_KERNEL);
	memset(handles->Handler,0,sizeof(handles->Handler[0]) * nrofobs);
	
	if (!handles->Handler) 
	    return PTP_RC_GeneralError;
	// *oinfos = calloc(sizeof((*oinfos)[0]),nrofobs);
	*oinfos = kmalloc(sizeof((*oinfos)[0]) * nrofobs,GFP_KERNEL);
	memset(*oinfos,0,sizeof((*oinfos)[0]) * nrofobs);
	
	if (!*oinfos) return PTP_RC_GeneralError;
	// *flags  = calloc(sizeof((*flags)[0]),nrofobs);
	*flags  = kmalloc(sizeof((*flags)[0]) * nrofobs,GFP_KERNEL);
	memset(*flags,0,sizeof((*flags)[0]) * nrofobs);
	...
	for (i=0;i<cnt;i++) {
		...
		if (ISOBJECT(cur))
			continue;
		for (j=0;j<handles->n;j++) 
		    if (nextchild == handles->Handler[j]) break;
		if (j == handles->n) 
		    continue;
