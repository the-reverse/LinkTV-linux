...
#ifdef CONFIG_USB_STORAGE_SPEEDUP
	int save_delay_use = delay_use;
#else
	int save_delay_use = delay_use;
#endif /* CONFIG_USB_STORAGE_SPEEDUP */
...
#ifdef CONFIG_USB_STORAGE_SPEEDUP
	if(!strcmp (us->pusb_dev->devpath, CONFIG_USB_STORAGE_SPEEDUP_DEVPATH))
	{
		...
		//if(platform_info.board_id != C0D_pvr_board)
			delay_use = CONFIG_USB_STORAGE_SPEEDUP_TIME;
	}
#else
	if(!strcmp (us->pusb_dev->devpath, platform_info.usb_internal_port))
	{
		delay_use = 3;
	}
#endif /* CONFIG_USB_STORAGE_SPEEDUP */
