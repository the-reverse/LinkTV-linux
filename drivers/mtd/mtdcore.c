...
static int mtdd_shutdown (struct dev *p_dev)
{
  
	printk("mtdd shutdown\n");
	int ret = 0, i;
	if (down_trylock(&mtd_table_mutex))
		return -EAGAIN;
	i = MAX_MTD_DEVICES-1;
	...
	for ( ; i >= 0; i--) {
			if (mtd_table[i] && mtd_table[i]->shutdown)
				mtd_table[i]->shutdown(mtd_table[i]);
		}
