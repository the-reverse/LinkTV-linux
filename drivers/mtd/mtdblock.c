...
//printk("---------[%d]----------\n", __LINE__);
	 ...
	 */
	 if ( mtd->type==MTD_NORFLASH||mtd->type == MTD_NANDFLASH || strstr(mtd->name, "nand") )
	 {
	...
	//	}else{
			...
			/* Partial sector: need to use the cache */
				...
				if (mtdblk->cache_state == STATE_EMPTY ||
					...
					mtdblk->cache_state = STATE_EMPTY;
					if ( mtd->type==MTD_NORFLASH||mtdblk->mtd->type == MTD_NANDFLASH || strstr(mtd->name, "nand") )
					{
						int nCnt=sect_size/mtdblk->mtd->oobblock;
						int idx = 0;
						retlen = 0;
						for(idx=0;idx<nCnt;idx++)
						{
							ret = MTD_READ(mtd, sect_start+idx*mtdblk->mtd->oobblock, mtdblk->mtd->oobblock, &retLenTmp, (unsigned char*)mtdblk->mtd->ptr_u32CacheAddr[idx]);
							retlen+=retLenTmp;
						}
					}
					else
				...
				if ( mtd->type==MTD_NORFLASH||mtdblk->mtd->type == MTD_NANDFLASH || strstr(mtd->name, "nand") )
				{
					int nStartAddrIdx = offset/mtdblk->mtd->oobblock;
					int nCnt=size/mtdblk->mtd->oobblock;
					int i=0;
					for(i=0;i<nCnt;i++)
					{
					  memcpy ( (char*)mtdblk->mtd->ptr_u32CacheAddr[nStartAddrIdx],buf+mtdblk->mtd->oobblock*i, mtdblk->mtd->oobblock);	
					  nStartAddrIdx++;
					}
				}
				else
	...
	while (len > 0) {
		...
		if (mtdblk->cache_state != STATE_EMPTY &&
			...
			if ( mtd->type==MTD_NORFLASH||mtdblk->mtd->type == MTD_NANDFLASH || strstr(mtd->name, "nand") )
			{
				...
				for(i=0;i<nCnt;i++)
				{
				  memcpy ( buf+mtdblk->mtd->oobblock*i,(char*)mtdblk->mtd->ptr_u32CacheAddr[nStartAddrIdx] ,mtdblk->mtd->oobblock);	
				  nStartAddrIdx++;
				}
			}
			else
			      ...
			      unsigned long block, char *buf)
{
	...
	if(!mtd->oobblock)
		mtd->oobblock = 512;
	addrSlots = mtdblk->mtd->erasesize/mtdblk->mtd->oobblock;
	if (unlikely(!mtdblk->cache_data && mtdblk->cache_size)) {
		if ( mtd->type==MTD_NORFLASH||mtd->type == MTD_NANDFLASH || strstr(mtd->name, "nand") )
		{
			mtdblk->mtd->ptr_u32CacheAddr=kmalloc(addrSlots*sizeof(int), GFP_KERNEL);
			for(i=0;i<addrSlots;i++)
			{
				mtdblk->mtd->ptr_u32CacheAddr[i]=(int)kmalloc(mtdblk->mtd->oobblock, GFP_KERNEL);
				if(!mtdblk->mtd->ptr_u32CacheAddr[i])
					return -EINTR;
			}
			mtdblk->cache_data  = (unsigned char*)mtdblk->mtd->ptr_u32CacheAddr;
		}
		else
		{
...
debug_mars("---------[%s]----------\n", __FUNCTION__);
	...
	if (!--mtdblk->count) {
		...
		if(mtdblk->mtd->ptr_u32CacheAddr)
		{
			for(i=0;i<mtdblk->mtd->erasesize/mtdblk->mtd->oobblock;i++)
				{
					if(mtdblk->mtd->ptr_u32CacheAddr[i])
					{
						kfree((unsigned int*)mtdblk->mtd->ptr_u32CacheAddr[i]);
						mtdblk->mtd->ptr_u32CacheAddr[i] = 0;
					}
				}
			if(mtdblk->mtd->ptr_u32CacheAddr)
			{
				kfree((unsigned int*)mtdblk->mtd->ptr_u32CacheAddr);
				mtdblk->mtd->ptr_u32CacheAddr = NULL;
			}
			mtdblk->cache_data=NULL;
		}
		if(mtdblk->cache_data)
