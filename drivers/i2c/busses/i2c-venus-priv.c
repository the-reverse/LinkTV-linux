...
{                      
    ...
    if (is_jupiter_cpu())
    {
        p_this->model_name = MODLE_NAME_JUPITER;
        ...
        {   
            ...
            if (cpu_rev_num()>=2)           // jupiter C version
            {         
                wr_reg(MIS_DUMMY1, WRITE_ENABLE1 | IC_DATA_DEL_EN(1) | IC_DATA_DEL(IC_DATA_DEL_2T));
            }
        }                
        else
        {            
        ...
        if (cpu_rev_num()>=2)           // jupiter C version
        {            
            wr_reg(MIS_DUMMY1, WRITE_ENABLE2 | IC1_DATA_DEL_EN(1) | IC1_DATA_DEL(IC_DATA_DEL_1T));                  
        }
        
        return 0;
    }        
        
    p_this->model_name = MODLE_NAME_MARS;
    ...
    // VENUS / NEPTUNE
    p_this->model_name = (is_neptune_cpu()) ? MODLE_NAME_NEPTUNE : MODLE_NAME_VENUS;            
...
{        
    ...
    if (!p_this->gpio_map.muxpad)            // set gpio mux pad if necessary...
        return;

    val = rd_reg(p_this->gpio_map.muxpad);     
    ...
    )
{
    ...
    venus_i2c_load_message(p_this, I2C_MASTER_READ, NULL, 0, rx_buf, rx_buf_len, 0);
    ...
    )
{
    ...
    if (hHandle!= NULL)
    {
        ...
        hHandle->gpio_map.scl         = gpio_scl;
        hHandle->gpio_map.sda         = gpio_sda;
