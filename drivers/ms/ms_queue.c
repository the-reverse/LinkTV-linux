 ...
 *    #000 2009-07-09 CMYu   modify from mmc_queue.c for ms usage
 */
...
/*
 * Prepare a MS request.  Essentially, this means passing the
...
static int ms_prep_request(struct request_queue *q, struct request *req)
{
	...
	if (req->flags & REQ_SPECIAL) {
		 ...
		 */
		blk_dump_rq_flags(req, "MS bad request");
	}
...
/**
 * ms_queue_suspend - suspend a MS request queue
 * @mq: MS queue to suspend
 *
...
/**
 * ms_queue_resume - resume a previously suspended MS request queue
 * @mq: MS queue to resume
 */
