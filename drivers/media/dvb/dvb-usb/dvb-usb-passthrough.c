...
#define STREAMER_BUFFER_FULL_PROTECTION
...
{    
...
#ifdef STREAMER_BUFFER_FULL_PROTECTION
    unsigned long remain_space = 0;
#endif
    ...
    {         
        ...
        if (streamer->p_rp > streamer->p_urb_wp)
        {
            remain_space = (unsigned long) streamer->p_rp - (unsigned long) streamer->p_urb_wp ;
        }
        else
        {            
            remain_space = (unsigned long) streamer->p_limit - (unsigned long) streamer->p_urb_wp;            
        }
        
        if (remain_space < streamer->urb_size)
        {
            dvb_streamer_warning("ring buffer fulled.........\n");         
            streamer->state |= STREAMER_BUFFER_FULL; 
        }
        
        if (streamer->state & STREAMER_BUFFER_FULL)
        {                        
            unlock_streamer(streamer);
            return;
        }            
    ...
    )
{
    ...
    {                                          
        ...
        if (streamer->state & STREAMER_BUFFER_FULL)        
            streamer->state &= ~STREAMER_BUFFER_FULL;            // remove buffer full flag        
#endif             
...
{    
    ...
    else
    {
        ...
        if (size)
        {                 
            unsigned char* p_buff_tmp = p_buff;
            p_buff = KSEG0ADDR(p_buff);         // convert it to kernel virtual address            
            ...
            streamer->p_user_kernel_ofst = p_buff_tmp - p_buff;
            ...
            dvb_streamer_dbg("set buffer : user kernel offset : %p\n",streamer->p_user_kernel_ofst);            
...
{        
    ...
    *p_size  = 0;
    ...
    {        
        ...
        *pp_buff += (unsigned long) streamer->p_user_kernel_ofst;
    ...
    unlock_streamer(streamer);        
        ...
        if (ret==0 && (streamer->state & STREAMER_BUFFER_FULL) && *p_size==0)
        {            
            dvb_usb_streamer_flush(streamer);            
        }            
...
{        
    ...
    p_buff -= streamer->p_user_kernel_ofst;
