...
#define MODEL_CONFIG_LEN	100 //64	// Originally this value is 20 for libra 10bytes config and now we support 32byes for customer extra use
//change 100 for IronEgg5000 ecd key
...
//	char vout_interface[1];			/* Video Out Interface */
	...
	char usb_internal_port[8];		/* For the boards booting from USB/Disk or USB/ISO, this is used to know what's the internal USB PORT. */
} platform_info_t;
...
#error This tree is not verified on Venus.
...
#ifdef CONFIG_REALTEK_CPU_NEPTUNE
#error This tree is not verified on Neptune.
	int cpu_id = platform_info.cpu_id;
...
#error This tree is not verified on Mars.
	int cpu_id = platform_info.cpu_id;
