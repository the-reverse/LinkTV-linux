...
// 
typedef enum _PTPObjectSessionState
{
  PtpObjectSessionClose    = 0,
  PtpObjectSessionOpen     = 1, 
  PtpObjectSessionData    = 2, 
} PTPObjectSessionState;

typedef struct _PTPObjectSession
{
    uint32_t handle;    
    ...
    unsigned long session_buf_offset;
    unsigned long session_buf_size;
    PTPObjectSessionState state;
} PTPObjectSession;
...
typedef uint16_t (* PTPDataPutFunc)	(PTPParams* params, void*priv,
					unsigned long sendlen,unsigned char *data, unsigned long *putlen);
	                                
typedef void *(* PTPDataGetDataPtrFunc)(PTPParams* params, void*priv);
typedef uint16_t (* PTPDataSetDataPtrFunc)(PTPParams* params, void*priv,void *data,unsigned long len);
typedef uint16_t (* PTPDataSetOffsetFunc)	(PTPParams* params, void*priv,unsigned long offset);
/*
typedef uint16_t (* PTPDataGetOffsetFunc)	(PTPParams* params, void*priv , unsigned long *offset);
typedef uint16_t (* PTPDataGetSizeFunc)	(PTPParams* params, void*priv,unsigned long *size);
*/
	          
typedef enum _PTPDataHandlerMode
{
  PtpDataHandlerModeNormal   = 0,
  PtpDataHandlerModeSession  = 1, 
} PTPDataHandlerMode;	          
...
typedef struct _PTPDataHandler {
	...
	PTPDataHandlerMode  mode;	
	
	PTPDataGetDataPtrFunc GetDataPtrFunc;
	PTPDataSetDataPtrFunc SetDataPtrFunc;
	PTPDataSetOffsetFunc  SetDataOffsetFunc;
	/*
	PTPDataGetOffsetFunc  GetDataOffsetFunc;
	PTPDataGetSizeFunc    GetDataSizeFunc;
	*/
	void			*priv;
} PTPDataHandler;
...
struct _PTPParams {
	...
	// Mutex
	struct semaphore session_mutex_lock;
	PTPObjectSession obj_session;
...
static int inline ptp_operation_issupported(PTPParams* params, uint16_t operation)
{
	int i=0;
...
extern uint16_t ptp_getobject_session(PTPParams *params, uint32_t handle, char *buf, uint32_t size, bool reset);
extern uint16_t ptp_getpartialobject_new(PTPParams *params, uint32_t handle, uint32_t offset, uint32_t maxbytes, unsigned char *object, uint32_t *len);
